...
export function shouldCompact(contextTokens, contextWindow, settings) {
    ...
    /* PI_COMPACT_CAP: PI_COMPACT_FULL_WINDOW_80 — no early automatic triggers. */

    if (!Number.isFinite(contextWindow) || contextWindow <= 0) return false;
    return Number.isFinite(contextTokens) && contextTokens >= Math.ceil(contextWindow * 0.8);
}
// ============================================================================
// Cut point detection
// ============================================================================
 ...
 */
export function getSummarizationFailure(response, label) {/* PI_SUMMARY_CONTENT_GUARD */
...
function createSummarizationOptions(model, maxTokens, apiKey, headers, env, signal, thinkingLevel, sessionId) {/* PI_SUMMARY_EFFORT */if(thinkingLevel&&thinkingLevel!=="off")thinkingLevel="low";
...
export function prepareCompaction(pathEntries, settings, contextWindow) {/* PI_COMPACT_WINDOW_SETTINGS */
settings = (function compactionSettingsForWindow(contextWindow, settings) {
