  ...
  event.type === 'message_end' && event.message?.role === 'assistant' && event.message.usage?.cost &&
    (event.message.usage.cost.billing = this.modelRuntime.isUsingSubscription(event.message.provider) ? 'subscription' : 'metered'), event));
        ...
        if (event.type === "message_end") {
            ...
            if (event.message.role === "assistant") {
                ...
                // PI_EMPTY_RESPONSE_POLICY (local patch; re-applied by verify-harness.mjs):
    ...
    _willRetryAfterAgentEnd(event) {
        ...
        for (let i = event.messages.length - 1; i >= 0; i--) {
            ...
            if (!/\b429\b|\b503\b|rate.?limit|too many requests|service.?unavailable|temporar(?:ily)? unavailable|upstream_unavailable|json error injected into sse stream|stream_read_error|finish_reason: error/i.test(message.errorMessage || ""))
                return this._retryAttempt < settings.maxRetries;
            const started = this._piRateLimitLoopStart;
            const elapsed = Date.now() - started;
            return !started || elapsed <= 120000 || elapsed > 480000;
        }
        return false;
    }
    ...
    async compact(customInstructions) {
        ...
        try {
            ...
            const settings = this.settingsManager.getCompactionSettings();
            const preparation = prepareCompaction(pathEntries,settings,this.model?.contextWindow)/* PI_COMPACT_WINDOW_CALL */;
            if (!preparation) {
            ...
            /* PI_COMPACTION_COMMIT_GUARD */if(typeof summary!=="string"||!summary.trim()||!Number.isFinite(tokensBefore)||tokensBefore<0||typeof firstKeptEntryId!=="string"||!this.sessionManager.getBranch().some(entry=>entry.id===firstKeptEntryId))throw new Error("Invalid compaction result; keeping existing history");this.sessionManager.appendCompaction(summary, firstKeptEntryId, tokensBefore, details, fromExtension, usage);
     ...
     */
    async _checkCompaction(assistantMessage, skipAbortedCheck = true) {/* PI_AUTO_WINDOW_CHECK */if(!this.model||!shouldCompact(estimateContextTokens(this.agent.state.messages).tokens,this.model.contextWindow,this.settingsManager.getCompactionSettings()))return false;
        const settings = this.settingsManager.getCompactionSettings();
        if (!settings.enabled)
            return false;
...
/* PI_AUTO_PRESERVE_FAILURE */return await this._runAutoCompaction("overflow",willRetry);
        }
     ...
     */
    async _runAutoCompaction(reason, willRetry) {/* PI_AUTO_WINDOW_RUN */if(!this.model||!shouldCompact(estimateContextTokens(this.agent.state.messages).tokens,this.model.contextWindow,this.settingsManager.getCompactionSettings()))return false;
        const settings = this.settingsManager.getCompactionSettings();
        let started = false;
        let fromExtension = false;
        try {
            ...
            const { model: requestModel, apiKey, headers, env } = await this._getSummarizationRequestAuth(this.model);
            const pathEntries = this.sessionManager.getBranch();
            const preparation = (prepareCompaction(pathEntries,settings,this.model?.contextWindow)/* PI_COMPACT_WINDOW_CALL */ || prepareCompaction(pathEntries,{...settings,keepRecentTokens:0},this.model?.contextWindow))/* PI_COMPACT_AUTO_TAIL */;
            if (!preparation) {
                return false;
            }
   ...
   try { await this._extensionRunner.emit(event);
     ...
     if(event.decision!==undefined) {
       if(event.decision==="retry") {
         this._flushPendingCustomMessages();this.agent.state.messages=this.agent.state.messages.filter(m=>m!==message);
         return true;
       }
       return false;
     }
   } finally { if(this._retryAbortController===controller)this._retryAbortController=undefined; }
 }
        ...
        // PI_RATE_LIMIT_POLICY (local patch; re-applied by verify-harness.mjs): cooldown-class
        ...
        const rateLimited = /\b429\b|\b503\b|rate.?limit|too many requests|service.?unavailable|temporar(?:ily)? unavailable|upstream_unavailable|json error injected into sse stream|stream_read_error|finish_reason: error/i.test(message.errorMessage || "");
        if (!rateLimited) {
            this._piRateLimitLoopStart = undefined;
        ...
        if (rateLimited && Date.now() - this._piRateLimitLoopStart > 120000) {
            /* PI_RETRY_PAUSE_RECORDED */ this.sessionManager.appendCustomEntry("harness-retry", {status: "paused", reason: "cooldown-window-exhausted", provider: message.provider, model: message.model, attempt: this._retryAttempt});
            message.errorMessage = (message.errorMessage || "429 rate limit") +
                " — paused: transient provider error persisted >2min; send a message to retry";
            this._piRateLimitLoopStart = undefined;
            return false;
        }
        this._retryAttempt++;
        ...
        // Wait with exponential backoff (abortable)
            ...
            /* PI_RETRY_CANCEL_RECORDED */ this.sessionManager.appendCustomEntry("harness-retry", {status: "cancelled", reason: "retry-cancelled", provider: message.provider, model: message.model, attempt: this._retryAttempt});
    ...
    getContextUsage(){/* PI_CONTEXT_SETTINGS */const usage=this._getContextUsageBeforeTelemetry();return usage?{...usage,compactionSettings:this.settingsManager.getCompactionSettings()}:usage;} _getContextUsageBeforeTelemetry(){
        const model = this.model;
        if (!model)
            return undefined;
