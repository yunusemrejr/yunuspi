...
function getPackageCommandUsage(command) {
    switch (command) {
        ...
        case "update":
            return `${APP_NAME} update --source <reviewed-checkout> | --extensions | --models | --extension <package-source>`;
        case "list":
...
Update YunusPi from reviewed source, explicitly installed extensions, or model catalogs.

Options:
  --source <checkout>     Apply reviewed YunusPi source through the owned installer
  --extensions            Update installed packages only
  --models                Refresh model catalogs only
  --all                   Legacy alias: update extensions and print core-update guidance
  --extension <source>    Update one package only
  -a, --approve           Trust project-local files for this command
  -na, --no-approve       Ignore project-local files for this command
  --force                 Reserved; use the reviewed YunusPi source installer to reinstall

Short forms:
  ${APP_NAME} update --source /path/to/yunuspi  Apply reviewed YunusPi source
  ${APP_NAME} update --extensions   Update installed extensions
  ${APP_NAME} update --models       Refresh model catalogs only
  ${APP_NAME} update <source>       Update one package
  ${APP_NAME} update                Show the owned source update workflow
`);
            return;
        case "list":
            console.log(`${chalk.bold("Usage:")}
...
export async function handlePackageCommand(args, runtimeOptions = {}) {
    if (args[0] === "update" && (args.length === 1 || args.includes("--source"))) {
        const result = spawnProcessSync(process.execPath, [join(getAgentDir(), "scripts/core-update.mjs"), ...args.slice(1)], { stdio: "inherit" });
    ...
    try {
        switch (options.command) {
            ...
            case "update": {
                ...
                if (updateTargetIncludesSelf(target)) console.log(getSelfUpdateUnavailableInstruction());
                return true;
            }
        }
    }
