...
yunuspi -r                  # Browse and select from past sessions
yunuspi --no-session        # Ephemeral mode; do not save
yunuspi --name "my task"    # Set session display name at startup
yunuspi --session <path|id> # Use a specific session file or partial session ID
yunuspi --fork <path|id>    # Fork a session file or partial session ID into a new session
```
...
`/resume` opens an interactive session picker for the current project. `yunuspi -r` opens the same picker at startup.
...
```bash
yunuspi --name "Refactor auth module"
yunuspi --name "CI audit" -p "Review this build failure"
```

Named sessions are easier to find in `/resume` and `yunuspi -r`.
