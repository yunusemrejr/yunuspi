...
```bash
yunuspi -c                  # Continue most recent session
yunuspi -r                  # Browse and select a session
yunuspi --no-session        # Ephemeral mode; do not save
yunuspi --name "my task"    # Set session display name at startup
yunuspi --session <path|id> # Use a specific session file or session ID
yunuspi --fork <path|id>    # Fork a session into a new session file
```
...
`yunuspi config` and package commands use the same project trust flow, except `yunuspi update` never prompts. Pass `--approve` to trust project-local settings for one command or `--no-approve` to ignore them.
...
```bash
yunuspi [options] [--] [@files...] [messages...]
```

### Package Commands

```bash
yunuspi install <source> [-l]     # Install package, -l for project-local
yunuspi remove <source> [-l]      # Remove package
yunuspi uninstall <source> [-l]   # Alias for remove
yunuspi update --extensions       # Update packages only; reconcile pinned git refs
yunuspi update --models           # Refresh model catalogs only
yunuspi update --extension <src>  # Update one package
yunuspi list                      # List installed packages
yunuspi config                    # Enable/disable package resources
```
...
```bash
yunuspi --no-extensions -e ./my-extension.ts
```
...
```bash
yunuspi @prompt.md "Answer this"
yunuspi -p @screenshot.png "What's in this image?"
yunuspi @code.ts @test.ts "Review these files"
```
...
yunuspi "List all .ts files in src/"

# Non-interactive
yunuspi -p "Summarize this codebase"

# Prompt beginning with a dash
yunuspi -p -- "- Summarize these points"

# Non-interactive with piped stdin
cat README.md | pi -p "Summarize this text"

# Named one-shot session
yunuspi --name "release audit" -p "Audit this repository"

# Different model
yunuspi --provider openai --model gpt-4o "Help me refactor"

# Model with provider prefix
yunuspi --model openai/gpt-4o "Help me refactor"

# Model with thinking level shorthand
yunuspi --model sonnet:high "Solve this complex problem"

# Limit model cycling
yunuspi --models "claude-*,gpt-4o"

# Read-only mode
yunuspi --tools read,grep,find,ls -p "Review the code"

# Disable one extension or built-in tool while keeping the rest available
yunuspi --exclude-tools ask_question
```
