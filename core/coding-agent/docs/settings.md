...
The fork does not contact upstream Pi version or install-telemetry endpoints. `enableInstallTelemetry` is retained as compatibility configuration and cannot enable an upstream core updater. `PI_OFFLINE=1` disables optional startup package/catalog network activity. Updates follow [YunusPi source releases](../../../docs/CORE-UPDATES.md).
