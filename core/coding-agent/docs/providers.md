...
```bash
export AZURE_OPENAI_API_KEY=${YOUR_CREDENTIAL}
...
export AWS_SECRET_ACCESS_KEY=${YOUR_CREDENTIAL}
...
```bash
yunuspi --provider amazon-bedrock --model us.anthropic.claude-sonnet-4-20250514-v1:0
```
...
yunuspi --provider amazon-bedrock --model arn:aws:bedrock:us-east-1:123456789012:application-inference-profile/abc123
```
...
yunuspi --provider cloudflare-ai-gateway --model "claude-sonnet-4-5"
```
...
export CLOUDFLARE_API_KEY=...           # or use /login
export CLOUDFLARE_ACCOUNT_ID=...
yunuspi --provider cloudflare-workers-ai --model "@cf/moonshotai/kimi-k2.6"
```
...
**Via extensions:** For providers that need custom API implementations or OAuth flows, create an extension. See [custom-provider.md](custom-provider.md) and [examples/extensions/custom-provider-gitlab-duo](https://github.com/earendil-works/pi/tree/d981de1229ef899957bbe968bc8dcda02a21f477/packages/coding-agent/examples/extensions/custom-provider-gitlab-duo/).
