...
async function mintApiKey(identityToken, signal) {
    ...
    if (response.status === 401 || response.status === 403) {
        ...
        throw new Error(`Meta session expired (status ${response.status}). Run \`/login meta\` to sign in again.${errorDetail(json)}`);
    }
    if (!response.ok) {
