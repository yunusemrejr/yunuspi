 ...
 *
 * Existing apps switch imports from "@yunuspi/ai" to
 * "@yunuspi/ai/compat" unchanged; new code uses `createModels()`
...
/** @deprecated Static catalog read. Use `getBuiltinModel` from "@yunuspi/ai/providers/all" or `Models.getModel()`. */
export const getModel = getBuiltinModel;
/** @deprecated Static catalog read. Use `getBuiltinModels` from "@yunuspi/ai/providers/all" or `Models.getModels()`. */
export const getModels = getBuiltinModels;
/** @deprecated Static catalog read. Use `getBuiltinProviders` from "@yunuspi/ai/providers/all" or `Models.getProviders()`. */
...
import { piWithStreamIdle } from "./utils/event-stream.js"; /* PI_STREAM_IDLE_IMPORT */
export function stream(model, context, options) { /* PI_STREAM_IDLE_CALL */ return piWithStreamIdle(model, options, idleOptions => piStreamIdleOriginalStream(model, context, idleOptions)); }
function piStreamIdleOriginalStream(model, context, options) {
...
export function streamSimple(model, context, options) { /* PI_STREAM_IDLE_CALL */ return piWithStreamIdle(model, options, idleOptions => piStreamIdleOriginalSimple(model, context, idleOptions)); }
function piStreamIdleOriginalSimple(model, context, options) {
    const builtinProvider = getBuiltinProviderForModel(model);
    if (builtinProvider) {
        if (model.provider.startsWith("cloudflare-") && !hasResolvedCloudflareAuth(options)) {
