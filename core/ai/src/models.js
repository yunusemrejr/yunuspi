...
class ModelsImpl {
    ...
    async refresh(options = {}) {
        ...
        const refresh = Promise.all(refreshable.map(async (provider) => {
            ...
            const operation = (async () => {
                ...
                // PI_CATALOG_REFRESH_CREDENTIAL_GATE: removed if(!credential)return; gate — remote catalog
    ...
    stream(model, context, options) { /* PI_STREAM_IDLE_CALL */ return piWithStreamIdle(model, options, idleOptions => this.piStreamIdleOriginalStream(model, context, idleOptions)); }
piStreamIdleOriginalStream(model, context, options) {
    ...
    streamSimple(model, context, options) { /* PI_STREAM_IDLE_CALL */ return piWithStreamIdle(model, options, idleOptions => this.piStreamIdleOriginalSimple(model, context, idleOptions)); }
piStreamIdleOriginalSimple(model, context, options) {
        return lazyStream(model, async () => {
            const provider = this.requireProvider(model);
            const { requestModel, requestOptions } = await this.applyAuth(model, options);
