...
import { appendList, branchTip, deleteList, deleteValue, entryLabel, list, pendingEntry, setValue, value, } from "../../values.js";
const MESSAGE_TIMESTAMP = 1_650_000_000_000;
const testName = value("test.session.name");
const testValue = (key) => value("test.value", key);
const testValuePrefix = (prefix = "") => value("test.value", prefix);
const testList = (key) => list("test.list", key);
...
function usage(input, output, options = {}) {
    return {
        input,
        output,
        cacheRead: input + 1,
...
function userEntry(id, parentId = null, text = id) {
    return {
        id,
        parentId,
        type: "message",
...
function customEntry(id, parentId, customType = "note", data = { id }) {
    return { id, parentId, type: "custom", customType, data };
}
function compactionEntry(id, parentId) {
    return {
        id,
        parentId,
        type: "compaction",
...
async function assertCommitStats(storage, result) {
...
export function createStorageConformance(factory) {
    return [
        createCase(factory, "transactions", "commits mixed writes atomically in write order", async ({ storage }) => {
            ...
            deepStrictEqual(await storage.getEntries(["entry"], BACKGROUND_CONTEXT), new Map([["entry", { ...userEntry("entry"), seq: result.seqs[0], timestamp: result.timestamp }]]));
        ...
        createCase(factory, "transactions", "rolls back every store when a mixed transaction fails", async ({ storage }) => {
            await storage.commit([insertEntry(userEntry("root")), insertUsage({ id: "taken", usage: usage(1, 1), adjustment: false })], BACKGROUND_CONTEXT);
        ...
        createCase(factory, "transactions", "preserves overwritten and deleted values when a transaction fails", async ({ storage }) => {
        ...
        createCase(factory, "transactions", "enforces one shared entry and usage id namespace", async ({ storage }) => {
            ...
            await rejects(storage.commit([insertUsage({ id: "existing-entry", usage: usage(2, 2), adjustment: false })], BACKGROUND_CONTEXT));
            ...
            deepStrictEqual((await storage.scanUsage({ order: "asc" }, BACKGROUND_CONTEXT)).map((row) => row.id), ["existing-usage"]);
        }),
        createCase(factory, "transactions", "resolves parents only from prior entries and earlier writes", async ({ storage }) => {
            await storage.commit([insertEntry(userEntry("root"))], BACKGROUND_CONTEXT);
            await storage.commit([insertEntry(customEntry("child", "root")), insertEntry(customEntry("grandchild", "child"))], BACKGROUND_CONTEXT);
            deepStrictEqual(ids(await storage.scanBranch({ start: "grandchild", order: "oldestFirst" }, BACKGROUND_CONTEXT)), ["root", "child", "grandchild"]);
            await rejects(storage.commit([
            ...
            await storage.commit([insertUsage({ id: "usage-is-not-parent", usage: usage(1, 1), adjustment: false })], BACKGROUND_CONTEXT);
            await rejects(storage.commit([insertEntry(customEntry("usage-child", "usage-is-not-parent"))], BACKGROUND_CONTEXT));
            deepStrictEqual(await storage.getEntries(["before-parent", "later-parent", "orphan", "usage-child"], BACKGROUND_CONTEXT), new Map());
        ...
        createCase(factory, "transactions", "places pending content under its reserved entry id", async ({ storage }) => {
            ...
            const placement = await storage.commit([insertEntry(entry), deleteValue(pendingEntry(entry.id)), setValue(branchTip("main"), entry.id)], BACKGROUND_CONTEXT);
            deepStrictEqual(await storage.getEntries([entry.id], BACKGROUND_CONTEXT), new Map([[entry.id, { ...entry, seq: placement.seqs[0], timestamp: placement.timestamp }]]));
        ...
        createCase(factory, "values", "sets, replaces, deletes, and recreates values without tombstones", async ({ storage }) => {
        ...
        createCase(factory, "values", "applies same-transaction value and list operations in write order", async ({ storage }) => {
        ...
        createCase(factory, "values", "does not change historical stores during value-only commits", async ({ storage }) => {
            await storage.commit([
            ...
            const usageBefore = await storage.scanUsage({ order: "asc" }, BACKGROUND_CONTEXT);
            const statsBefore = await storage.getStats(BACKGROUND_CONTEXT);
            const result = await storage.commit([setValue(testName, "first"), setValue(testName, "second")], BACKGROUND_CONTEXT);
            deepStrictEqual(await storage.scanEntries({ order: "asc" }, BACKGROUND_CONTEXT), entriesBefore);
            deepStrictEqual(await storage.scanUsage({ order: "asc" }, BACKGROUND_CONTEXT), usageBefore);
            deepStrictEqual(await storage.getStats(BACKGROUND_CONTEXT), statsBefore);
            deepStrictEqual(await storage.getValue(testName, BACKGROUND_CONTEXT), {
        ...
        createCase(factory, "lists", "pages appends by global sequence and deletes whole lists", async ({ storage }) => {
            ...
            const result = await storage.commit([appendList(address, "a"), setValue(testName, "gap"), appendList(address, "b"), appendList(address, "c")], BACKGROUND_CONTEXT);
            ...
            deepStrictEqual(await storage.readList(address, { cursor: { seq: result.seqs[0] }, limit: 2 }, BACKGROUND_CONTEXT), [
                { seq: result.seqs[2], value: "b" },
                { seq: result.seqs[3], value: "c" },
            ]);
            ...
            deepStrictEqual(await storage.readList(address, { order: "desc", cursor: { seq: result.seqs[3] }, limit: 2 }, BACKGROUND_CONTEXT), [
                { seq: result.seqs[2], value: "b" },
                { seq: result.seqs[0], value: "a" },
            ]);
            ...
            await storage.commit([deleteList(address), deleteList(testList("absent")), appendList(address, "new")], BACKGROUND_CONTEXT);
            deepStrictEqual((await storage.readList(address, undefined, BACKGROUND_CONTEXT)).map(({ value }) => value), ["new"]);
        }),
        createCase(factory, "lists", "clamps one read page without limiting list growth", async ({ storage }) => {
            const address = testList("large");
            await storage.commit(Array.from({ length: 10_001 }, (_, index) => appendList(address, index)), BACKGROUND_CONTEXT);
            ...
            strictEqual((await storage.readList(address, {
                cursor: { seq: firstPage.at(-1).seq },
            }, BACKGROUND_CONTEXT)).length, 1_000);
        }),
        createCase(factory, "lists", "commits mixed list writes atomically and rolls them back with siblings", async ({ storage }) => {
            ...
            await rejects(storage.commit([appendList(address, "transient"), deleteValue(testName), insertEntry(userEntry("mixed"))], BACKGROUND_CONTEXT));
            deepStrictEqual(await storage.readList(address, undefined, BACKGROUND_CONTEXT), [
                { seq: committed.seqs[1], value: "kept" },
            ]);
        ...
        createCase(factory, "entry queries", "stores custom entries with and without data", async ({ storage }) => {
            ...
            deepStrictEqual(await storage.getEntries(["without-data", "with-data"], BACKGROUND_CONTEXT), new Map([
                [
                    "without-data",
                    {
                        id: "without-data",
        ...
        createCase(factory, "entry queries", "scans global entries with explicit ranges, filters, orders, and limits", async ({ storage }) => {
            const result = await storage.commit([
                insertEntry(userEntry("root")),
            ...
            deepStrictEqual(ids(await storage.scanEntries({
                type: "custom",
                customType: "note",
                fromSeq: result.seqs[1],
                toSeq: result.seqs[3],
                order: "desc",
            }, BACKGROUND_CONTEXT)), ["note-2", "note-1"]);
        ...
        createCase(factory, "branch queries", "applies stops before filters and cursors before limits", async ({ storage }) => {
            const result = await storage.commit([
                insertEntry(userEntry("root")),
            ...
            deepStrictEqual(ids(await storage.scanBranch({ start: "leaf", stopAtType: "compaction", type: "message" }, BACKGROUND_CONTEXT)), ["leaf"]);
            deepStrictEqual(ids(await storage.scanBranch({
                start: "leaf",
                order: "oldestFirst",
                stopAtId: "middle",
                type: "custom",
            }, BACKGROUND_CONTEXT)), ["marker"]);
            deepStrictEqual(ids(await storage.scanBranch({
                start: "leaf",
                order: "newestFirst",
                cursor: { seq: result.seqs[4] },
                limit: 2,
            }, BACKGROUND_CONTEXT)), ["compact", "middle"]);
            deepStrictEqual(ids(await storage.scanBranch({
                start: "leaf",
                order: "oldestFirst",
                cursor: { seq: result.seqs[1] },
                limit: 2,
            }, BACKGROUND_CONTEXT)), ["middle", "compact"]);
            deepStrictEqual(ids(await storage.scanBranch({ start: "leaf", stopAtId: "leaf", type: "custom" }, BACKGROUND_CONTEXT)), []);
        ...
        createCase(factory, "branch queries", "returns branch structure without payload fields", async ({ storage }) => {
            const result = await storage.commit([insertEntry(userEntry("root")), insertEntry(customEntry("child", "root", "note"))], BACKGROUND_CONTEXT);
        ...
        createCase(factory, "branch queries", "applies branch query semantics to structure scans", async ({ storage }) => {
            ...
            deepStrictEqual(ids(await storage.scanBranchStructure({ start: "leaf", stopAtType: "compaction", type: "message" }, BACKGROUND_CONTEXT)), ["leaf"]);
            deepStrictEqual(ids(await storage.scanBranchStructure({ start: "leaf", order: "oldestFirst", cursor: { seq: result.seqs[1] }, limit: 2 }, BACKGROUND_CONTEXT)), ["middle", "compact"]);
            await rejects(storage.scanBranchStructure({ start: "missing" }, BACKGROUND_CONTEXT));
        }),
        createCase(factory, "usage and stats", "scans the usage ledger with explicit ranges, orders, and limits", async ({ storage }) => {
            const result = await storage.commit([
            ...
            deepStrictEqual((await storage.scanUsage({ fromSeq: result.seqs[1], toSeq: result.seqs[2], order: "asc" }, BACKGROUND_CONTEXT)).map((row) => row.id), ["usage-2"]);
            deepStrictEqual((await storage.scanUsage({ order: "desc", limit: 2 }, BACKGROUND_CONTEXT)).map((row) => row.id), ["usage-3", "usage-2"]);
            deepStrictEqual((await storage.scanUsage({ order: "asc", limit: 2 }, BACKGROUND_CONTEXT)).map((row) => row.id), ["usage-1", "usage-2"]);
        }),
        createCase(factory, "usage and stats", "keeps stats equal to message count and ledger totals", async ({ storage }) => {
        ...
        createCase(factory, "lifecycle", "seals admission, drains admitted commits, and closes idempotently", async ({ storage }) => {
            ...
            for (const read of rejectedReads)
                await rejects(read);
        }),
    ];
}
