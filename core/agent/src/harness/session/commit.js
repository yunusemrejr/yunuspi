...
export function insertUsage(row) {
    return { kind: "usage", row };
}
export function commitWrite(write, seq, timestamp) {
    switch (write.kind) {
        case "entry":
...
export function materializeCommittedEntry(entry, seq, timestamp) {
    return { ...entry, seq, timestamp };
}
export function prepareStorageCommit(writes, firstSeq, timestamp) {
...
export function validateCommittedWrites(writes, firstSeq, state) {
    ...
    const transactionEntryIds = new Set();
    for (const write of writes) {
        if (write.seq <= previousSeq)
            throw new Error(`Non-monotonic storage sequence: ${write.seq}`);
        ...
        if (write.kind === "entry")
            transactionEntryIds.add(write.id);
    }
}
