import { AssistantMessageFrameEncoder, isContextOverflow, isRecoverableLength, isRetryableAssistantError, } from "@yunuspi/ai";
...
import { branchTip, deleteList, operationPreparation, pendingAssistantFrames, setValue } from "../../session/values.js";
import { openFrameProgress } from "../progress.js";
import { retryDelay, retryNotBefore } from "./retry.js";
import { prepareOverflowCompaction } from "./structural.js";
import { operationCleanupWrites, operationResultRecord } from "./terminal.js";
export function openAssistantResponse(lane, drive, responseEntryId, recovery = false) {
    ...
    return {
        observer: {
            ...
            update(message, event, context) {
                const frame = frameEncoder.encode(event);
                if (frame !== undefined)
                    progress.write(frame);
                return lane.emitBatch([{ type: "message_update", ...eventContext, message, event, ...(frame === undefined ? {} : { frame }) }], context);
            },
            end(message, context) {
                return lane.emitBatch([{ type: "message_end", ...eventContext, message, entryId: responseEntryId }], context);
            },
        },
        async afterResponse(message, metadata, context) {
            await close();
            const result = await lane.hooks.runWithGate("after_response", { lane: lane.name, runId: drive.operationId, ...metadata, message }, drive.gate, context);
...
export async function publishConfigurationFailure(lane, drive, capability, error) {
...
function uuidV7Timestamp(id) {
    ...
    if (!Number.isSafeInteger(timestamp))
        throw new SessionInvariantError(`Invalid reserved UUIDv7 ${id}`);
    return timestamp;
}
function providerError(source, message) {
    return {
...
function normalizeError(message, errorMessage) {
    return { ...message, stopReason: "error", errorMessage };
}
function normalizeAborted(source, message) {
    return {
        ...message,
        stopReason: "aborted",
        errorMessage: message.errorMessage ?? `${source === "assistant" ? "Assistant" : "Deferred"} request was cancelled`,
    };
}
function deferredHandleIsValid(message, generation) {
    const handle = message.deferred;
    const identity = generation.generationContext.configuration.model;
    return (message.stopReason === "deferred" &&
        handle !== undefined &&
        handle.id.length !== 0 &&
...
export async function publishResponse(lane, drive, intent, response, options = {}) {
    const overflow = intent.at === "assistant.effect_pending" &&
        (isContextOverflow(response, intent.contextWindow) || isRecoverableLength(response, intent.intendedOutputLimit));
    const overflowPreparation = overflow && !intent.generationContext.overflowRecoveryUsed
        ? await prepareOverflowCompaction(lane, drive, intent)
        : undefined;
    return lane.settleOperation(intent, async (state, current, meta, reader) => {
        const source = current.at === "assistant.effect_pending" ? "assistant" : "deferred";
        const responseEntryId = intent.responseEntryId;
        const configuration = current.at === "assistant.effect_pending" ? current.generationContext.configuration : current.configuration;
        const turnId = current.at === "assistant.effect_pending"
        ...
        if (current.control.status === "cancel_requested") {
            ...
            throw new SessionInvariantError(`${source === "assistant" ? "Assistant" : "Deferred"} response is aborted while durable control is running`);
        }
        else if (current.at === "assistant.effect_pending" && overflow) {
            committed = normalizeError(response, response.errorMessage ?? "Assistant request exceeded the context window");
            ...
            const calls = response.content.flatMap((content, sourceIndex) => content.type === "toolCall" ? [{ sourceIndex }] : []);
            if (calls.length !== 0) {
                const timestamp = uuidV7Timestamp(responseEntryId);
                const planned = calls.map(({ sourceIndex }) => ({
                    status: "planned",
                    sourceIndex,
        ...
        const record = failure === undefined ? undefined : operationResultRecord(meta, "failed", responseEntryId, failure);
        const cleanup = record === undefined ? [] : await operationCleanupWrites(reader, drive.operationId, current, drive.context);
        const writes = [
            ...
            ...(settled?.at === "summary.deciding" && overflowPreparation !== undefined
                ? [
                    setValue(operationPreparation(drive.operationId, overflowPreparation.taskId), overflowPreparation.preparation),
                ]
                : []),
        ];
        const materializeEntry = (commit) => ({
            ...responseEntry,
            seq: commit.seqs[0],
        ...
        const events = (commit) => {
                ...
                if (options.recovery !== true && current.attempt > 1 && settled?.at !== "assistant.retry_wait") {
                    ...
                    batch.push({
                        ...
                        ...(success
                            ? {}
                            : {
                                finalError: committed.errorMessage ?? `Assistant request ended with ${committed.stopReason}`,
                            }),
                    });
                }
            ...
            if ((current.at === "deferred.effect_pending" || options.recovery !== true) &&
        ...
        if (settled === undefined)
            throw new SessionInvariantError("Response settlement is missing its next state");
        return {
            kind: "commit",
            writes,
