...
import { deleteList, deleteValue, operationMeta, operationPreparationPrefix, operationState as operationStateValue, operationToolArgsPrefix, operationToolMemoPrefix, pendingAssistantFrames, pendingEntry, pendingToolOutputPrefix, } from "../../session/values.js";
/** Build the mechanical operation-owned suffix used by an owning procedure's terminal transaction. */
export async function operationCleanupWrites(reader, operationId, state, context) {
    ...
    if (state.at === "tools") {
        for (const call of state.batch.calls) {
            if (call.status === "outcome_ready")
                pendingIds.add(call.resultEntryId);
        }
    }
    let frameDelete;
...
export function operationResultRecord(meta, status, tipId, error) {
