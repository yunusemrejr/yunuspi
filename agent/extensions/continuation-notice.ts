...
import { collectContinuationLines, continuationWarning } from "./lib/continuation-notice.ts";
...
export default function continuationNoticeExtension(pi: ExtensionAPI): void {
  pi.on("message_end", (event, ctx) => {
    ...
    const warning = continuationWarning(collectContinuationLines(), ctx.hasPendingMessages());
    if (!warning) return undefined;
    let content: unknown;
    if (typeof message.content === "string") content = message.content + warning;
