...
export function collectSessionMetrics(entries, live) {
 ...
 for(const [i,e] of entries.entries()) {
  ...
  if(e.type==='custom'&&e.customType==='span-usage-v1'){
   const d=e.data??{};
   m.span.hits++;if(d.cached===true)m.span.cached++;if(d.shadow===true)m.span.shadow++;
   m.span.tokens+=number(d.inputTokens);m.span.present+=number(d.present);
   if(typeof d.costUsd==='number'&&d.costUsd>=0)m.span.costUsd+=d.costUsd;
  }
