...
export function collectHarnessUsage(entries:any[],live?:any) {
  ...
  for(const entry of Array.isArray(entries)?entries:[]) {
    ...
    if(entry?.type==='custom'&&['session-observer-delivery-v1','watchmaker-delivery-v1'].includes(entry.customType)&&typeof entry.data?.adviceId==='string') {
  ...
  const requests=collectAuxiliaryModelUsage(entries).rows.filter((row:any)=>['session-observer','session-watchmaker'].includes(row.owner));
...
export function harnessUsageHtml(data:ReturnType<typeof collectHarnessUsage>) {
  ...
  const components=data.components.map(row=>`<details class="usage-component"><summary><span><b>${esc(row.name)}</b><small>${row.name==='JEV'?'Remote structured judge':row.name==='Needle3'?'Local WASM model':row.name==='Fuzzy matching'?'Local lexical matching · JavaScript':row.name==='WASM source check'?'Tree-sitter WASM parser':'Local helper'}</small></span><span class="usage-component-value">${row.measured?`${count(row.executions)} recorded executions · ${count(row.applied)} applied`:row.displayed?`${count(row.displayed)} displayed notices`:'No measurement in this snapshot'}</span></summary>${row.measured?`<div class="usage-mini-grid">${metric(row.results,'Results ready')}${metric(row.delivered,'Context deliveries')}${metric(row.returned,'Excerpt returns')}${metric(row.cached,'Recorded cached/shared events')}${metric(row.skipped,'Skipped / abstained')}${metric(row.failed,'Unavailable / failed')}</div>${facts([['Measured latency',row.timingSamples?`${Math.round(row.durationMs/row.timingSamples)} ms average · ${count(row.timingSamples)} samples · ${Math.round(row.durationMs)} ms total`:'not recorded'],['Operations',Object.entries(row.operations).map(([k,v])=>`${k}: ${v}`).join(' · ')||'not recorded'],['Reasons',Object.entries(row.reasons).map(([k,v])=>`${k}: ${v}`).join(' · ')||'none recorded'],...(row.name==='Fuzzy matching'?[['Matched candidates',`${count(row.matches)} total across recorded operations`]]:[])])}`:`<p class="note">${row.displayed?'Only displayed notices survived in this branch; display deduplication prevents reconstructing total calls or timing.':'No component measurements were retained by this snapshot’s capture time. Later activity is not included; missing measurements do not prove the component was disabled or never ran.'}</p>`}</details>`).join('');
