  ...
  const api = {
    ...
    result(event: any, ctx: any) {
      ...
      if (['render_see','design_audit','browser_session'].includes(event.toolName) && !event.isError && Array.isArray(event.details?.noise?.findings)) {
        const keys: Record<string,string> = {'decorative-dot-marker':'ui-dot-marker','icon-tile':'ui-icon-tile','repeated-heavy-left-border':'ui-accent-rail','animated-status-pill':'ui-live-pill','eyebrow-pill':'ui-eyebrow-pill','text-status-dot':'ui-dot-marker','factual-claim-evidence':'prose-metric-theater'};
