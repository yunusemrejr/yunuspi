  ...
  const api = {
    ...
    result(event: any, ctx: any) {
      ...
      if (['render_see','design_audit','browser_session'].includes(event.toolName) && !event.isError && Array.isArray(event.details?.noise?.findings)) {
        const keys: Record<string,string> = {'decorative-dot-marker':'ui-dot-marker','icon-tile':'ui-icon-tile','repeated-heavy-left-border':'ui-accent-rail','missing-image-alt':'ui-missing-alt','unnamed-control':'ui-icon-button-name','emoji-in-chrome':'ui-emoji-ui','hype-badge-cluster':'ui-badge-spam','fake-terminal-decoration':'ui-fake-terminal'};
