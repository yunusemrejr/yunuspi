...
export const HARNESS_CAPABILITIES: readonly HarnessCapability[] = Object.freeze([
	...
	capability({
		id: "art-direction",
		group: "web_media",
		summary: "Close the creative loop around one shared direction. creative_direct owns the structured brief (intent, focal hierarchy, visual bounds, avoid-list, motion and audio character); visual_review captures real renders and records revision-sensitive rubric verdicts; ui_explore renders the viewport/state matrix with DOM facts; motion_inspect inventories animations and samples temporal frames; svg_inspect measures geometry and set consistency; asset_register keeps provenance, roles and reuse search; image_generate briefs and calls a configured OpenAI-compatible backend with review routing; creative_compare renders direction variants side by side. Blocking verdicts hold the completion gate.",
		...
		options: [
			...
			option("viewports|states", "Matrix axes: mobile/tablet/desktop widths × default/dark/reduced-motion/full states, capped at 12 captures."),
