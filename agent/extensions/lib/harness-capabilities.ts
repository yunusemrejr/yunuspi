...
export const HARNESS_CAPABILITIES: readonly HarnessCapability[] = Object.freeze([
	...
	capability({
		...
		commands: [
			"cost", "self", "metrics", "obs", "effort", "reminder", "graph", "provider", "or-provider",
		...
		options: [
			...
			option("/sys-prompt|/used|/commands", "Separate-window popups: opening system prompt, session usage, command list."),
	...
	capability({
		...
		group: "coordination",
		summary: "Coordinate independent Pi sessions sharing one checkout through voluntary objectives, file scopes, recent writes and a local Markdown board; peers are advisory, not remote messaging or locks.",
		entrypoints: ["session_coordinate", "sibling-bridge", "todo-plan-changed"],
		tools: ["session_coordinate"],
		options: [
			option("action", "Coordination operation.", ["status", "publish", "clear"]),
			option("objective", "Bounded objective text, up to 240 characters."),
			option("note", "Bounded handoff note, up to 500 characters."),
			option("files", "Up to 32 absolute or cwd-resolved files/directories."),
	...
	capability({
		...
		tools: ["evidence_cache", "context_score", "handoff_capsule"],
		options: [
	...
	capability({
		id: "web-and-media",
		group: "web_media",
		summary: "Search and read web sources, diagnose pages, inspect browser state and analyze or transform local media through bounded extension tools.",
