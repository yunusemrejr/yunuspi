...
try {
  ...
  let result;
  if (['sqlite_probe', 'archive_probe', 'workspace_search', 'local_mail_search', 'local_mail_read'].includes(name)) {
  ...
  else if (name === 'ssh_plan') result = (await import('./net.mjs')).sshPlan(args);
  else {
