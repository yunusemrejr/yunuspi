...
const LEAK_WEAK = ['react','vue','angular','svelte','next.js','nuxt','tailwind','docker','kubernetes','sqlite','postgres','mongodb','redis','graphql','rest api','pipeline','schema','embeddings','orchestration','vercel','netlify','cloudflare workers','terraform','webpack','vite','system prompt','chain of thought','context window','training data','fine-tuned','token limit','prompt engineering','reasoning model','base model','temperature'];
...
/** First-person agent narration and meta-design commentary leaked into reader copy. */
const WORK_THOUGHT = /(?:as an ai\b|i (?:designed|built|created|crafted|chose|picked|went with)|this (?:section|design|redesign|page|site|hero|component) (?:showcases|demonstrates|highlights|embodies|uses)|built to (?:showcase|demonstrate)|in this (?:redesign|iteration|version|concept) (?:i|we)|here'?s what (?:i|we)|my (?:design|approach|take)|(?:i|we) (?:kept|used|chose) (?:the|a) (?:minimal|clean|modern|playful|bold) (?:look|feel|aesthetic|design))/i;
...
const BUZZWORDS = ['next-generation','next-gen','state-of-the-art','world-class','best-in-class','revolutionary','cutting-edge','seamless','robust','scalable','innovative','intelligent','contextual','frictionless','holistic','synergy','synergies','game-changer','game-changing','supercharge','supercharges','unleash','unlock','elevate','empower','leverage','streamline','transformative','ai-driven','data-driven','mission-critical','lightning-fast','rock-solid'];
...
const VAGUE_NAV_LABEL = 'insights|discover|explore|solutions|resources|platform|our platform|features|use cases|why us|what we do|company';
/** Status words models invent for decorative pills and dots. Motion or a pill
 * enclosure must also be present; the word alone never fires. */
const STATUS_WORD = 'live|online|real[ -]?time|active|operational|connected|running|streaming|available|ready|enabled|working|tracking|recording|sync(?:ing|ed)|in progress|now live|live now|system (?:online|active|operational)|all systems operational|ai (?:online|active|live|powered)|beta|new';
const PILL_CLASS = 'rounded-full|rounded-\\[9999px\\]|rounded-\\[50%\\]|\\b(?:status-)?pill\\b|\\bbadge\\b|\\bchip\\b|\\bcapsule\\b';
...
export function slopGuidanceSignals(file: string, value: unknown, limit = 3): CodeSignal[] {
  ...
  if (ui || script) {
    ...
    const statusRe = new RegExp(`\\b(?:${STATUS_WORD})\\b`, 'i');
    const pillRe = new RegExp(`\\b(?:${PILL_CLASS})`, 'i');
    const motionRe = new RegExp(`\\b(?:${MOTION_CLASS})`, 'i');
    for (const match of text.matchAll(/<(?:span|div|p|a|button|li)\b[^>]{0,500}>[\s\S]{0,1200}?<\//gi)) {
      const region = match[0];
      if (statusRe.test(region) && pillRe.test(region) && motionRe.test(region)) {
        add('ui-live-pill', 'A status label, pill enclosure and blinking/pulsing treatment occur together. Remove this decorative status badge by default; prefer quiet, specific status text tied to observed state. If live activity is required, verify connected, stale, offline and reduced-motion states. A source cue cannot prove the status is fake.');
        break;
      }
    }
    // Nested shape: the label sits after inner closed elements
    // (<div class="badge"><i class="pulsing-dot"></i>Online</div>), past the
    // first closing tag above. Same three-part test, one cue at most.
    if (!out.some(s => s.key === 'ui-live-pill')
        && new RegExp(`<(?:span|div|p|a|button|li)\\b[^>]{0,500}(?:${PILL_CLASS})[^>]{0,500}>[\\s\\S]{0,600}(?:${MOTION_CLASS})[\\s\\S]{0,600}?\\b(?:${STATUS_WORD})\\b\\s*<\\/`, 'i').test(text))
      add('ui-live-pill', 'A status label, pill enclosure and blinking/pulsing treatment occur together. Remove this decorative status badge by default; prefer quiet, specific status text tied to observed state. If live activity is required, verify connected, stale, offline and reduced-motion states. A source cue cannot prove the status is fake.');
    // Glowing/pulsing dots without the full pill: the dual-span ping pattern,
    // keyframed blink/pulse/glow loops, and literal dot characters before a
    // status word. A static dot alone is ui-dot-marker; motion or glow makes
    // it this cue. The ping pattern and invented labels yield to a fired
    // live pill: one cue per badge keeps reminders and review sharing the
    // same signal instead of crowding each other out.
    const livePill = out.some(s => s.key === 'ui-live-pill');
    if (!livePill && /\banimate-ping\b/i.test(text) && /\brounded-full\b/i.test(text))
      add('ui-glow-dot', 'An animate-ping span pairs with a rounded dot (the stock glowing status-dot pattern). Remove the ping/glow by default; a real state indicator is quiet text, or motion that stops once the state is known. If the state is genuinely live, verify stale, offline and reduced-motion rendering.');
    else if (new RegExp(`@keyframes\\s+[\\w-_]*(?:blink|puls|glow|ping|breathe)[\\w-_]*(?:\\{[\\s\\S]{0,800}?\\})?\\s*[\\s\\S]{0,2000}?animation[^;]{0,120}infinite`, 'i').test(text) || /animation(?:-name)?\s*:[^;}\n]{0,120}(?:blink|puls|glow|ping)/i.test(text))
      add('ui-glow-dot', 'A blink/pulse/glow/ping keyframe loop is declared. Remove decorative glowing or blinking dots by default; keep looping motion only for a real, changing state with a text equivalent, and verify reduced-motion behavior.');
    if (new RegExp(`[●•]\\s*(?:${STATUS_WORD})`, 'i').test(text) || /(?:🟢|🟡|🔴)\s*(?:live|online|active|operational|ready|new|beta)\b/i.test(text))
      add('ui-glow-dot', 'A literal dot character (●/•/emoji) prefixes a status word. This is the text form of the decorative status dot; remove it and state the fact plainly. Keep a marker only for a real, changing state with an accessible text equivalent.');
    // Invented static status labels: BETA/NEW/LIVE/AI POWERED pills and badges
    // with no evident backing state. Animated ones already trip ui-live-pill.
    if (!livePill && /<(?:span|div|p|a|button|li)\b[^>]{0,300}(?:pill|badge|rounded-full|chip|capsule)[^>]*>[\s\S]{0,120}?\b(?:BETA|NEW|LIVE|AI[ -]?POWERED|COMING SOON|STAY TUNED|V\d[.\d]*)\b/i.test(text))
      add('ui-fake-status-label', 'A pill/badge presents an invented status label (BETA, NEW, LIVE, AI POWERED, COMING SOON). Remove it unless a real fact stands behind it — a version, a dated release, observed live data. Badges are state machinery, not ornament.');
    // Eyebrow pills: a pill/badge kicker above a heading ("✨ Introducing X"),
    // or clusters of label pills that decorate instead of selecting/filtering.
    if (new RegExp(`<(?:span|div|p)\\b[^>]{0,300}(?:${PILL_CLASS})[^>]{0,300}>[\\s\\S]{0,200}?<\\/(?:span|div|p)>[\\s\\S]{0,600}?<h[12]\\b`, 'i').test(text)
        || /(?:✨|🚀|⚡|🎉|💡|🤖)\s*(?:introducing|announcing|meet|new:?)\b/i.test(text))
      add('ui-eyebrow-pill', 'A pill/badge kicker sits above a heading (often "✨ Introducing X"). Remove the eyebrow pill; let the heading state the point. Kickers earn their place only when they add information the heading lacks (a version, a date, a scope).');
    else {
      const pillAttrs = text.match(/class(?:Name)?\s*=\s*"[^"]*(?:\bpill\b|\bbadge\b)[^"]*"/gi) ?? [];
      if (pillAttrs.length >= 3)
        add('ui-eyebrow-pill', `${pillAttrs.length} pill/badge-class enclosures occur in this source. Pills must select, filter or encode state; remove label pills for things that are not filters or states, and any cluster that decorates rather than organizes.`);
    }
  ...
  if (ui) {
    ...
    const dots = blocks.filter(b => { const w = px(b.body, 'width'), h = px(b.body, 'height'); return w !== undefined && h !== undefined && w >= 4 && w <= 16 && Math.abs(w - h) <= 2 && /border-radius\s*:\s*(?:50%|100%|9{3,}px|\d{2,}px)/i.test(b.body) && /background(?:-color)?\s*:/i.test(b.body); }).length
      + classLists.filter(list => has(list, /\s(?:size|[wh])-(?:1|1\.5|2|2\.5|3|3\.5|4)\s/) && has(list, /\srounded-(?:full|\[50%\]|\[9999px\])\s/) && has(list, /\sbg-[a-z]/)).length
      + classLists.filter(list => has(list, /\s[wh]-\[(?:[4-9]|1[0-6])px\]\s/) && has(list, /\srounded-(?:full|\[50%\]|\[9999px\])\s/) && has(list, /\sbg-[a-z]/)).length;
    if (dots)
  ...
  if (prose) {
    ...
    if (!inDocs) {
      ...
      if (WORK_THOUGHT.test(visible))
        push('prose-work-thought-leak','copywriting','Agent work-thought leaks into reader copy (first-person build narration, "this section showcases", "as an AI"). Readers came for the outcome, not the making-of commentary. Cut the narration; keep only the fact it was wrapping.');
    }
    if (buzzHits >= 4)
    ...
    if (!inDocs) {
      ...
      const tells = report.words >= 120 ? report.findings.filter(f=>['stock-phrase','dash-density','rule-of-three','repeated-opener','boilerplate-heading','slop-label'].includes(f.rule)) : [];
    ...
    if (navLabels.size >= 2)
      push('prose-vague-nav','copywriting','Two or more navigation labels are vague (Insights, Discover, Explore, Solutions, Resources, Platform, Features, Use Cases, Why Us, What We Do, Company). Name each destination specifically; generic labels force every visitor to click to learn what lives there.');
  }
