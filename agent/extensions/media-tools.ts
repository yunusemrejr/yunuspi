...
export default function mediaTools(pi: any) {
  ...
  const style = choices(['studio', 'clay', 'toon', 'wireframe']);
  ...
  const sceneObject = Type.Object({ id: Type.String({ pattern: '^[a-zA-Z][\\w-]{0,47}$' }), parent: Type.Optional(Type.String({ maxLength: 48 })), geometry: Type.Optional(choices(['box', 'sphere', 'torus', 'torusKnot', 'cone', 'cylinder', 'icosahedron', 'plane', 'group'])), position: Type.Optional(positionVector), rotation: Type.Optional(vector), scale: Type.Optional(scaleVector), color: Type.Optional(hex), roughness: Type.Optional(Type.Number({ minimum: 0, maximum: 1 })), metalness: Type.Optional(Type.Number({ minimum: 0, maximum: 1 })), opacity: Type.Optional(Type.Number({ minimum: 0.05, maximum: 1 })) });
  ...
  const scene = Type.Object({ version: Type.Optional(Type.Literal(1)), title: Type.Optional(Type.String({ maxLength: 80 })), duration: Type.Optional(Type.Number({ minimum: 0.1, maximum: 30 })), ...resolution, style: Type.Optional(style), background: Type.Optional(hex), exposure: Type.Optional(Type.Number({ minimum: 0.1, maximum: 3 })), camera: Type.Optional(Type.Object({ position: Type.Optional(positionVector), target: Type.Optional(positionVector), fov: Type.Optional(Type.Number({ minimum: 15, maximum: 100 })) })), lights: Type.Optional(Type.Array(Type.Object({ type: Type.Optional(choices(['hemisphere', 'directional', 'point'])), color: Type.Optional(hex), ground: Type.Optional(hex), position: Type.Optional(positionVector), intensity: Type.Optional(Type.Number({ minimum: 0, maximum: 100 })) }), { minItems: 1, maxItems: 4 })), objects: Type.Array(sceneObject, { minItems: 1, maxItems: 64 }), tracks: Type.Optional(Type.Array(Type.Object({ target: Type.String({ maxLength: 48 }), property: Type.Optional(choices(['position', 'rotation', 'scale', 'target', 'fov'])), keys: Type.Array(animationKey, { minItems: 2, maxItems: 64 }) }), { maxItems: 128 })) });
  register('scene_create', 'Author a real local Three.js 3D scene as editable JSON plus self-contained HTML (bundled Three.js, no network). Provide scene or an orbital/kinetic/sculpture preset, then edit the JSON. Studio/clay/toon/wireframe styles, parented geometry, camera/lights and absolute-time keyframes. Rotation is radians; ease applies after its key. Maximum 64 objects, 512 keys, 30 seconds, 900 frames and 900 million pixel-frames. Creation is not render evidence.', Type.Object({ scene: Type.Optional(scene), preset: Type.Optional(choices(['orbital', 'kinetic', 'sculpture'])), style: Type.Optional(style), ...outputSchema }), sceneCreate);
