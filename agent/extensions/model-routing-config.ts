...
import { resolveLlmPreferenceChain, resolvePromptAnalysisPreferenceChain, resolveSessionObserverPreferenceChain } from "./pi-subagents/src/runs/shared/model-fallback.ts";
...
function entryDisplay(doc,raw,index){const r=rowRoute(doc,raw);if(state.role==='session_observer'){r.model=r.model.replace(/:(off|minimal|low|medium|high|xhigh|max)$/,'');const prefix=r.provider+'/';if(r.model.toLowerCase().startsWith(prefix.toLowerCase()))r.model=r.model.slice(prefix.length);r.fullId=r.provider?r.provider+'/'+r.model:r.model}const m=state.data.models.find(x=>x.fullId.toLowerCase()===r.fullId.toLowerCase());return {r,m,index}}
...
function formatResolution(role){const info=state.data.resolvedByRole?.[role]||{source:'autonomous',routes:[]};if(role==='session_observer')return info.status==='disabled'?'Observer disabled. Reset role restores the default.':info.status==='unavailable'?(info.reason||'Observer route unavailable; no fallback.'):info.source==='default'?'Default: official DeepSeek Flash, high thinking.':'Uses this ordered list only; no automatic model fallback.';if(role==='prompt_analysis'&&info.source==='subagents')return 'No explicit Prompt Analysis list; inheriting Subagents priority. Reset returns to this behavior.';if(info.source==='autonomous')return 'No viable configured route. Existing model selection applies.';return 'Effective source: '+(info.source==='prompt_analysis'?'Prompt Analysis':roleLabel(info.source))}
...
function render(){renderRoles();const doc=currentDoc(),role=state.role,list=listFor(doc,role),roleDef=state.data.roles.find(x=>x.id===role);const activeRoleKey=configRoleKey(doc,role),explicit=Object.hasOwn(doc.preferences||{},activeRoleKey);const effective=formatResolution(role);const reloadButton='<button type="button" id="reload">Reload latest JSON</button>';const restore=state.data.backupAvailable?'<button type="button" id="restore">Restore previous file</button>':'';let html='<div class="section-head"><div><h2>'+escAttr(roleDef?.label||role)+'</h2><p>'+escAttr(roleDef?.detail||'Specialized model role.')+'</p></div><div class="actions">'+reloadButton+'<button type="button" id="reset" '+(!state.data.ok?'disabled':'')+'>Reset role</button>'+(role==='session_observer'?'<button type="button" id="disable-observer">Disable observer</button>':'')+restore+'</div></div>';
...
html+=role==='session_observer'?'<p class="muted">'+escAttr(effective)+'</p>':notice(effective,'');if(role==='prompt_analysis'&&!explicit)html+=notice('Prompt Analysis inherits the Subagents ordered routes until you add a dedicated list. Clearing this role restores inheritance.','');
...
async function updateThinking(index,value){await edit(doc=>{const list=listFor(doc,state.role);const entry=mutableEntry(doc,list,index);entry.model=String(entry.model).replace(/:(off|minimal|low|medium|high|xhigh|max)$/,'');entry.thinking=value;return rewriteRole(doc,list)},'Observer thinking saved.')}
...
async function chooseModel(model){const doc=clone(currentDoc()),list=listFor(doc,state.role),item={provider:model.provider,model:model.id,...(state.role==='session_observer'?{thinking:'high'}:{}),...(model.selectedProviderOptions?{provider_options:model.selectedProviderOptions}:model.selectedProviderRouting?{provider_options:{routing:'pinned',order:model.selectedProviderRouting.order||model.selectedProviderRouting.only}}:{})};const replaceAt=state.replaceIndex;const duplicateIndex=list.findIndex((raw,index)=>index!==replaceAt&&sameRouteEntry(doc,raw,item));if(duplicateIndex>=0){state.replaceIndex=null;setStatus('That exact route is already priority '+(duplicateIndex+1)+' in '+roleLabel(state.role)+'.','error');render();return}if(replaceAt!==null){const old=rowRoute(doc,list[replaceAt]).entry;list[replaceAt]={...(old&&typeof old==='object'?old:{}),...item,...(state.role==='session_observer'&&old?{thinking:observerThinking(old)}:{})};if(!model.selectedProviderRouting)delete list[replaceAt].provider_options;state.replaceIndex=null}else{list.push(item)}rewriteRole(doc,list);await save({document:doc},'Route saved to '+roleLabel(state.role)+'. '+(model.availabilityLabel||''))}
