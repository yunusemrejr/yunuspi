...
import { scoreModelRoutingSearch, compareModelRoutingSearch, modelRoutingProviderHints, readModelRoutingSnapshot, saveModelRoutingSnapshot } from "./lib/model-routing-store.ts";
import { fetchEndpoints, type Endpoint } from "./pi-subagents/src/runs/shared/openrouter-endpoints.ts";
import { resolveLlmPreferenceChain, resolvePromptAnalysisPreferenceChain, resolveSessionObserverPreferenceChain, resolveWatchmakerPreferenceChain } from "./pi-subagents/src/runs/shared/model-fallback.ts";
...
const BUILTIN_ROLES = [
	...
	{ id: "session_observer", label: "Session Observer", detail: "Brief periodic observations about tool use, skills and coordination." },
	{ id: "watchmaker", label: "Mr. Watchmaker", detail: "Timekeeper: periodic time-vs-progress notes about waste and faster paths." },
...
function resolutionFor(ctx: ExtensionContext, role: string, configPath = llmPreferencesPath()) {
	...
	const options = { configPath, diagnostics: "inspect" as const, onSkip: (event: typeof skipped[number]) => skipped.push(event) };
	if (role === "session_observer") {
		const result = resolveSessionObserverPreferenceChain(available, options);
		return { ...result, skipped };
	}
	if (role === "watchmaker") {
		const result = resolveWatchmakerPreferenceChain(available, options);
		return { ...result, skipped };
	}
...
const canonicalRole=x=>({main:'main_session_fallback',mainfallback:'main_session_fallback',mainsessionfallback:'main_session_fallback',subagent:'subagents',swarms:'swarm',councils:'council',qualityreview:'quality_review',qualityreviews:'quality_review',projectreview:'project_review',projectreviews:'project_review',errorreview:'error_review',errorreviews:'error_review',bugreview:'error_review',bugreviews:'error_review',promptanalysis:'prompt_analysis',initialintentanalysis:'prompt_analysis',followupintentanalysis:'prompt_analysis',sessionobserver:'session_observer',mrwatchmaker:'watchmaker'}[roleId(x)]||roleId(x));
const FIXED_DEFAULT_ROLES=new Set(['session_observer','watchmaker']);
const isFixedDefault=role=>FIXED_DEFAULT_ROLES.has(role);
const shortRoleName=role=>role==='session_observer'?'Observer':role==='watchmaker'?'Watchmaker':roleLabel(role);
...
function entryDisplay(doc,raw,index){const r=rowRoute(doc,raw);if(isFixedDefault(state.role)){r.model=r.model.replace(/:(off|minimal|low|medium|high|xhigh|max)$/,'');const prefix=r.provider+'/';if(r.model.toLowerCase().startsWith(prefix.toLowerCase()))r.model=r.model.slice(prefix.length);r.fullId=r.provider?r.provider+'/'+r.model:r.model}const m=state.data.models.find(x=>x.fullId.toLowerCase()===r.fullId.toLowerCase());return {r,m,index}}
function observerThinking(entry){return String(entry.model||'').match(/:(off|minimal|low|medium|high|xhigh|max)$/)?.[1]||String(entry.thinking||'auto').trim().toLowerCase().replace(/^none$/,'off').replace(/^dynamic$/,'auto')}
...
function formatResolution(role){const info=state.data.resolvedByRole?.[role]||{source:'autonomous',routes:[]};if(isFixedDefault(role)){const name=shortRoleName(role);return info.status==='disabled'?name+' disabled. Reset role restores the default.':info.status==='unavailable'?(info.reason||name+' route unavailable; no fallback.'):info.source==='default'?'Default: official DeepSeek Flash, '+(role==='watchmaker'?'low':'high')+' thinking.':'Uses this ordered list only; no automatic model fallback.';}if(role==='prompt_analysis'&&info.source==='subagents')return 'No explicit Prompt Analysis list; inheriting Subagents priority. Reset returns to this behavior.';if(info.source==='autonomous')return 'No viable configured route. Existing model selection applies.';return 'Effective source: '+(info.source==='prompt_analysis'?'Prompt Analysis':roleLabel(info.source))}
function modelFacts(item){const cost=item.cost;const unknown=cost?.missing?.length||(!cost?.knownFree&&(!Number.isFinite(cost?.input)||!Number.isFinite(cost?.output)||cost.input===0&&cost.output===0));return [item.contextWindow?item.contextWindow.toLocaleString()+' context':null,item.maxTokens?item.maxTokens.toLocaleString()+(item.outputLimitEstimated?' output allowance (limit unverified)':' max output'):'output limit unknown',item.reasoning===true?'reasoning':null,unknown?'price unknown':cost?.knownFree?'known free':null]}
function renderRouteRows(doc,list){if(!list.length)return '<div class="empty">'+(isFixedDefault(state.role)?'Add a route to customize the '+shortRoleName(state.role).toLowerCase()+'. An empty saved list disables it.':'No configured priorities. The existing autonomous selection applies.')+'</div>';return list.map((raw,index)=>{const {r,m}=entryDisplay(doc,raw,index),entry=r.entry||{},opts=entry.provider_options||{},mode=String(opts.routing||'auto').toLowerCase(),slugs=(Array.isArray(opts.order)?opts.order:Array.isArray(opts.only)?opts.only:[]).filter(x=>typeof x==='string').join(', '),resolved=state.data.resolvedByRole?.[state.role]?.routes||[],effectiveRouting=state.data.routingByRole?.[state.role]?.[index],isLive=resolved.some(x=>x.route.toLowerCase()===r.fullId.toLowerCase()&&stableRouting(x.providerRouting)===stableRouting(effectiveRouting));const meta=m?[(m.availabilityLabel||(m.enabled?'Available in this session':'Catalog only · provider not configured here')),m.providerHost?m.providerHost:null,...modelFacts(m)].filter(Boolean).join(' · '):'Not in the current model registry';const custom=mode==='custom',pinned=mode==='pinned';return '<div class="route-row" data-index="'+index+'" data-route="'+escAttr(r.fullId)+'"><div class="rank">'+(index+1)+'</div><div><div class="route-name">'+escAttr(m?.name||r.model||r.fullId)+(index===0&&!isFixedDefault(state.role)?'<span class="first">first choice</span>':'')+'</div><div class="route-id">'+escAttr(r.fullId)+'</div><div class="route-meta">'+escAttr(meta)+(r.alias?' · alias '+escAttr(r.alias):'')+'</div></div><div class="route-meta">'+escAttr(pinSummary(effectiveRouting||m?.globalProviderRouting))+(isLive?' · resolver accepts this route':' · not in current viable chain')+'</div><div class="row-actions"><button class="small" data-action="up" aria-label="Move '+escAttr(r.fullId)+' up" '+(index===0?'disabled':'')+'>↑</button><button class="small" data-action="down" aria-label="Move '+escAttr(r.fullId)+' down" '+(index===list.length-1?'disabled':'')+'>↓</button><button class="small" data-action="replace">Replace</button><button class="small danger" data-action="remove">Remove</button></div><div class="settings">'+(isFixedDefault(state.role)?'<label>Thinking <select data-field="thinking" aria-label="'+shortRoleName(state.role)+' thinking">'+['auto','off','minimal','low','medium','high','xhigh','max'].map(level=>'<option value="'+level+'" '+(observerThinking(entry)===level?'selected':'')+'>'+level+'</option>').join('')+'</select></label>':'')+(r.provider.toLowerCase()==='openrouter'?'<label>Backend routing <select data-field="routing"><option value="auto" '+(mode==='auto'?'selected':'')+'>Automatic</option><option value="pinned" '+(pinned?'selected':'')+'>Pinned (no fallback)</option><option value="custom" '+(custom?'selected':'')+'>Preferred order</option></select></label><label>Provider slug(s) <input type="text" data-field="order" value="'+escAttr(slugs)+'" placeholder="Use a provider ID from /provider" '+(!custom&&!pinned?'disabled':'')+'></label><label><input type="checkbox" data-field="fallbacks" '+(effectiveRouting?.allow_fallbacks===true?'checked':'')+' '+(!custom?'disabled':'')+'> Allow fallback providers</label><span class="muted">'+(pinned?'Pinned routes always disable fallback providers.':custom?'Existing only, ignore, sort, and other provider options are preserved.':'Switch to preferred order to edit upstream preferences.')+'</span>':'')+'</div></div>'}).join('')}
function renderResults(){const box=$('#search-results');const active=document.activeElement;const activeRow=active?.closest?.('.result');const activeKey=activeRow?.dataset.key;box.replaceChildren();if(!state.query.trim())return;if(state.searchWarning){const warning=document.createElement('div');warning.className='notice warning';warning.textContent=state.searchWarning;box.append(warning)}for(const [index,item] of state.searchRows.entries()){const row=document.createElement('div');row.className='result';row.dataset.resultIndex=String(index);row.dataset.key=JSON.stringify([item.fullId,item.selectedProviderRouting||null]);const meta=[item.routeAvailability,item.providerHost,item.availabilityLabel||(item.enabled?'available':'not enabled'),...modelFacts(item)].filter(Boolean).join(' · ');const directName=item.provider==='deepseek'?'DeepSeek Official API':item.provider==='friendli'?'FriendliAI':item.providerHost?item.provider+' · '+item.providerHost:item.provider+' direct API';const routeName=item.upstreamName?'OpenRouter → '+item.upstreamName:item.provider==='openrouter'?'OpenRouter · automatic upstream':directName;const variant=item.selectedProviderRouting?pinSummary(item.selectedProviderRouting):item.provider==='openrouter'?(item.globalProviderRouting?pinSummary(item.globalProviderRouting):'Upstream not selected'):'Official provider route';row.innerHTML='<div class="result-main"><div class="result-title">'+escAttr(item.name||item.id)+'</div><div class="result-route">'+escAttr(routeName)+' · '+escAttr(item.fullId)+'</div><div class="result-meta">'+escAttr(variant)+' · '+escAttr(meta)+'</div></div><button type="button">'+(state.replaceIndex===null?'Add':'Replace')+(item.availability==='unconfigured'?' (not configured)':item.availability==='blocked'||!item.enabled?' (unavailable)':'')+'</button>';row.querySelector('button').addEventListener('click',()=>void chooseModel(item));box.append(row)}if(!state.searchRows.length){const empty=document.createElement('div');empty.className='empty';empty.textContent='No matching routes. Search uses the locally available model registry and verified route metadata.';box.append(empty)}if(activeKey){const next=$$('.result',box).find(row=>row.dataset.key===activeKey)?.querySelector('button');next?.focus()}}
function render(){renderRoles();const doc=currentDoc(),role=state.role,list=listFor(doc,role),roleDef=state.data.roles.find(x=>x.id===role);const activeRoleKey=configRoleKey(doc,role),explicit=Object.hasOwn(doc.preferences||{},activeRoleKey);const effective=formatResolution(role);const reloadButton='<button type="button" id="reload">Reload latest JSON</button>';const restore=state.data.backupAvailable?'<button type="button" id="restore">Restore previous file</button>':'';let html='<div class="section-head"><div><h2>'+escAttr(roleDef?.label||role)+'</h2><p>'+escAttr(roleDef?.detail||'Specialized model role.')+'</p></div><div class="actions">'+reloadButton+'<button type="button" id="reset" '+(!state.data.ok?'disabled':'')+'>Reset role</button>'+(isFixedDefault(role)?'<button type="button" id="disable-observer">Disable '+shortRoleName(role).toLowerCase()+'</button>':'')+restore+'</div></div>';
...
html+=isFixedDefault(role)?'<p class="muted">'+escAttr(effective)+'</p>':notice(effective,'');if(role==='prompt_analysis'&&!explicit)html+=notice('Prompt Analysis inherits the Subagents ordered routes until you add a dedicated list. Clearing this role restores inheritance.','');
...
function renderTrace(role){const info=state.data.resolvedByRole?.[role]||{source:'autonomous',routes:[],skipped:[]},routes=info.routes||[],skipped=info.skipped||[];const items=skipped.map(item=>'<li><strong>Priority '+escAttr(item.priority)+' skipped:</strong> <code>'+escAttr(item.route)+'</code> · '+escAttr(item.reason)+'</li>');if(!routes.length&&!items.length)return '<div class="empty">'+(isFixedDefault(role)?(info.status==='disabled'?shortRoleName(role)+' is disabled.':'No '+shortRoleName(role).toLowerCase()+' request will run until its route is available.'):'No viable configured route. The existing resolver falls back to normal model selection.')+'</div>';for(const [index,route] of routes.entries())items.push('<li><strong>'+(index===0?'Selected when available':'Fallback '+(index+1))+':</strong> <code>'+escAttr(route.route)+'</code>'+(route.providerRouting?' · '+escAttr(pinSummary(route.providerRouting)):'')+'</li>');return '<ol>'+items.join('')+'</ol>'}
function bindCommon(){$('#reload')?.addEventListener('click',()=>void refreshLatest());$('#restore')?.addEventListener('click',()=>void save({restoreBackup:true}));$('#reset')?.addEventListener('click',()=>void edit(doc=>{const key=configRoleKey(doc,state.role);delete doc.preferences[key];return doc},'Role reset; inherited/default routing applies.'));$('#disable-observer')?.addEventListener('click',()=>void edit(doc=>rewriteRole(doc,[]),shortRoleName(state.role)+' disabled.'))}
function bindRows(){const root=$('#editor');for(const row of $$('.route-row',root)){const index=Number(row.dataset.index);for(const button of $$('button[data-action]',row))button.addEventListener('click',()=>void rowAction(index,button.dataset.action));for(const control of $$('[data-field]',row))control.addEventListener('change',()=>void (control.dataset.field==='thinking'?updateThinking(index,control.value):updateRouteOption(index,row)))}}
...
async function updateThinking(index,value){await edit(doc=>{const list=listFor(doc,state.role);const entry=mutableEntry(doc,list,index);entry.model=String(entry.model).replace(/:(off|minimal|low|medium|high|xhigh|max)$/,'');entry.thinking=value;return rewriteRole(doc,list)},shortRoleName(state.role)+' thinking saved.')}
...
async function chooseModel(model){const doc=clone(currentDoc()),list=listFor(doc,state.role),item={provider:model.provider,model:model.id,...(isFixedDefault(state.role)?{thinking:state.role==='watchmaker'?'low':'high'}:{}),...(model.selectedProviderOptions?{provider_options:model.selectedProviderOptions}:model.selectedProviderRouting?{provider_options:{routing:'pinned',order:model.selectedProviderRouting.order||model.selectedProviderRouting.only}}:{})};const replaceAt=state.replaceIndex;const duplicateIndex=list.findIndex((raw,index)=>index!==replaceAt&&sameRouteEntry(doc,raw,item));if(duplicateIndex>=0){state.replaceIndex=null;setStatus('That exact route is already priority '+(duplicateIndex+1)+' in '+roleLabel(state.role)+'.','error');render();return}if(replaceAt!==null){const old=rowRoute(doc,list[replaceAt]).entry;list[replaceAt]={...(old&&typeof old==='object'?old:{}),...item,...(isFixedDefault(state.role)&&old?{thinking:observerThinking(old)}:{})};if(!model.selectedProviderRouting)delete list[replaceAt].provider_options;state.replaceIndex=null}else{list.push(item)}rewriteRole(doc,list);await save({document:doc},'Route saved to '+roleLabel(state.role)+'. '+(model.availabilityLabel||''))}
	...
	const server = createServer((req, res) => {
		void (async () => {
			...
			if (path === `/${token}/api/search`) {
				...
				const providerNames = [...new Set(snapshot.models.map(model => model.provider))];
				const ranked = snapshot.models.map(model => {
					const evidence = routeAliasEvidence(evidenceIndex, model);
					const score = Math.max(scoreModelRoutingSearch(query, model, evidence.aliases, evidence.pins, providerNames), genericQuery ? scoreModelRoutingSearch(genericQuery, model, evidence.aliases, evidence.pins, providerNames) : 0);
					return { model, score, evidence };
				}).filter(item => item.score > 0).sort(compareModelRoutingSearch).slice(0, hints.length ? 8 : 60);
				...
				const resultModels = [...variants, ...ranked].sort(compareModelRoutingSearch).slice(0, 60).map(item => item.model);
