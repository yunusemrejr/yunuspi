...
export function scoreContext(items: ContextItem[], task: string, now = Date.now()) {
  ...
  return items.map((item, index) => {
    ...
    const words = terms(item.text), matches = [...query].filter(x => words.has(x)).length;
    const relevance = matches / Math.sqrt(Math.max(1, query.size) * Math.max(1, words.size));
    ...
    return { ...item, score, protected: protectedItem, reasons: { relevance, freshness, provenance: !!item.source }, index };
