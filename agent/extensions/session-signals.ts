...
// measurements refresh when this file has no connection to the running session.
const USED_SNAPSHOT_SCRIPT = `<script>(function(){var stamp=document.getElementById('usage-captured-at'),age=document.getElementById('usage-snapshot-age');if(!stamp||!age)return;var captured=Date.parse(stamp.dateTime);function update(){if(!Number.isFinite(captured)){age.textContent='Capture time unavailable';return;}var seconds=Math.floor((Date.now()-captured)/1000);if(seconds<0){age.textContent='Capture clock is ahead of this browser';return;}var n=seconds<60?seconds:seconds<3600?Math.floor(seconds/60):seconds<86400?Math.floor(seconds/3600):Math.floor(seconds/86400);var unit=seconds<60?'second':seconds<3600?'minute':seconds<86400?'hour':'day';age.textContent=n+' '+unit+(n===1?'':'s')+' old';}update();setInterval(update,1000);document.addEventListener('visibilitychange',update);})();</script>`;
