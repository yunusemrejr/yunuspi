...
function maybeReadSkillMetadata(filePath: string): Record<string, string> | undefined {
	try {
		return parseFrontmatter(fs.readFileSync(filePath, "utf-8")).frontmatter;
	} catch {
...
function collectFilesystemSkills(cwd: string, agentDir: string, skillPaths: SkillSearchPath[]): CachedSkillEntry[] {
	...
	const pushEntry = (name: string, filePath: string, sourceHint?: SkillSource) => {
		...
		if (!description) return; // Match native Pi: ordinary Markdown is not a skill.
...
export function resolveSkills(
	...
	for (const trimmed of new Set(skillNames.map(name => name.trim()))) {
		if (!trimmed) continue;
