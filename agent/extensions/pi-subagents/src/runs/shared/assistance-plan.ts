...
export function selectAssistanceTeam(models: ModelInfo[], config: ModelEconomyConfig, plan: AssistancePlan, options: {freeOnly?:boolean;task?:string} = {}): AssistanceMember[] {
 ...
 const pool = models.filter(m=>(m.contextWindow??0)>=16384 && (m.maxTokens??0)>=1024 && catalogRouteCapabilities(m)?.toolCalling===true);
 ...
 for (const role of plan.roles.slice(0,3)) {
  ...
  const available = (freeOnly:boolean, diverse:boolean) => selectAffordableModel(diverse ? candidates.filter(m=>!team.some(member=>member.route.startsWith(m.provider+"/"))) : candidates,cheap,
   {freeOnly,quality:{...taskQuality(options.task),level:"advisory"},requirements:{minContextWindow:16384,minOutputTokens:1024,reasoning:false,inputModalities:["text"]}});
