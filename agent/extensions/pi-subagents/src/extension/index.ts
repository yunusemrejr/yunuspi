...
export default function registerSubagentExtension(pi: ExtensionAPI): void {
	...
	const asyncCompleteHandler = (payload: unknown) => {
		try { persistSubagentCost(pi, state, payload); } catch (error) { console.error("Subagent cost persistence failed:", error); }
	...
	const resetSessionState = (ctx: ExtensionContext, recovering: boolean, previousSessionFile?: string) => {
		...
		try { restoreSubagentCosts(pi, state, ctx.sessionManager.getEntries(), id => readCompletionReplay(DIRS.results, id, {sessionId:state.currentSessionId ?? undefined})); } catch (error) { console.error("Subagent cost restoration failed:", error); }
