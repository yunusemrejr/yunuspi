...
export const SUBAGENT_TOOL_PROMPT_GUIDELINES = [
	"Use subagent for useful independent work; honor requests not to delegate. Use one async workflow or declarative task set, await its calls, and keep one writer per worktree. Select relevant skills per child. Resolve requested model/provider/thinking with action=models and an exact model='provider/id:thinking'; never silently substitute. Completion notifications wake the parent; do not poll. Use a subagent for bounded independent work by default; use a swarm for separable investigations and fusion for competing approaches. Before launching, account for automatic helpers already announced in this turn. Prefer capable proven-free routes, with at most one cheap complementary helper in automatic teams. Quality evidence, task fit, route health and cost govern selection; preserve disagreements and verify advisory conclusions. Set thinking to the task requirements, not price alone.",
];
