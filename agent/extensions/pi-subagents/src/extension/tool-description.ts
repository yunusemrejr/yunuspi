...
Run inspection: use {action:"status",id:"run-id"}; add view:"transcript" for retained output or view:"fleet" for an overview. get requires agent and reads its definition. Resume selects a zero-based child index and requires its saved launch contract; an explicit resume may revive a stopped child. Never automatically restart a stopped child.
...
export const SUBAGENT_TOOL_PROMPT_GUIDELINES = [
	"Use subagent for useful independent work; honor requests not to delegate. Use one async workflow or declarative task set, await its calls, and keep one writer per worktree. Select relevant skills per child. Resolve requested model/provider/thinking with action=models and an exact model='provider/id:thinking'; never silently substitute. Completion notifications wake the parent; do not poll. Default to bounded delegation; swarms suit separable investigations, fusion suits competing approaches. Account for announced helpers. Prefer capable proven-free routes; automatic teams allow at most one cheap complement. Select by quality evidence, task fit, route health and cost. Preserve disagreements, verify conclusions, and match thinking to the task requirements.",
];
