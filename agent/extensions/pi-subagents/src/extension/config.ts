...
function validateForkContextConfig(value: unknown): void {
	...
	if (config.mode !== undefined && config.mode !== "full" && config.mode !== "pruned" && config.mode !== "capsule") {
		throw new Error('config.forkContext.mode must be "full", "pruned" or "capsule"');
	}
