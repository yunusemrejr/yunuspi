...
export function findContent(
	...
	// Edge-case hardenings: a non-string `text` (e.g. a fetch whose content field
	// was left undefined) used to throw a TypeError out of .toLocaleLowerCase()
	// with no error path for the caller; and non-string query entries crashed
	// .trim(). Both now degrade to a well-formed "no matches" result instead.
	if (typeof text !== "string") {
		return { text: `Text matches (${mode}): no matches`, matchCount: 0, returnedMatches: 0, queryResults: [] };
	}
	const normalizedQueries = [...new Set(queries.map(query => typeof query === "string" ? query.trim() : "").filter(Boolean))];
