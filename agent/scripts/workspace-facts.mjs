...
const reviewSkip = part => (TEST_SKIP.test(part) || REVIEW_RUNTIME_DIR.test(part)) && !/^(?:\.github|\.circleci|\.gitlab-ci\.yml)$/.test(part);
...
const REVIEW_RUNTIME_DIR = /^(?:reminders?|checkpoints?|sibling-bridge|tasks?|memory|missions?|project-intelligence|npm|cache|inference-slots?)$/i;
const REVIEW_RUNTIME_STATE = /^(?:provider-health|free-route-evidence|live-model-catalog|models-store|run-history|session-report|notification-state)\.(?:json|jsonl)$/i;
