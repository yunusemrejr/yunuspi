...
function classification(file) {
  ...
  if (
    /(^|\/)(uploads?|storage|logs?|data|db|var|dist|build|coverage)(\/|$)|\.(db|sqlite3?|dump|bak)$/i.test(
      file,
    )
  )
    return "runtime/generated candidate";
  return "unclassified";
}
function publicRemote(line) {
  ...
  const raw = m[3];
  let host = null,
    transport = "local-or-custom (not inspected)";
  try {
    const u = new URL(raw);
    if (["https:", "http:", "ssh:", "git:"].includes(u.protocol)) {
      host = u.hostname.slice(0, 160);
      transport = u.protocol.slice(0, -1);
    }
  } catch {
    const scp = /^(?:[^@\s]+@)?([a-zA-Z0-9.-]+):[^/].*$/.exec(raw);
    if (scp) {
      host = scp[1].slice(0, 160);
      transport = "ssh";
    }
  }
  return {
...
export async function workspaceFacts(cwd, signal) {
  ...
  const result = {
    ...
    source: "fresh local filesystem and read-only Git",
    manifests: [],
    directories: [],
    commands: [],
    dependencyEvidence: [],
    ...
    remoteState:
      "not inspected; remote configuration is not production evidence",
    synchronization: "not authorized or assessed",
    truncated: false,
    limits: {
      markerPaths: MARKERS.length + DIRS.length + RUNTIME.length,
      gitCommands: 13,
      gitOutputBytesPerCommand: 16384,
      changedPaths: 40,
      worktrees: 10,
      manifestBytes: 65536,
      outputChars: 12000,
      scopeProbes: 160,
      scopeCandidates: 24,
      scopeAncestorLevels: 4,
      scopeEntriesPerContainer: 64,
    },
    limitations: [
      "Filename classifications are hints, not ownership or safety decisions.",
      "Local, repository and production are not assumed equal. Databases, uploads, secrets and generated state are not mergeable source by default.",
      "No remote probes, hooks, deployment commands, Git diff filters or manifest scripts are executed.",
  ...
  for (const name of RUNTIME)
    if (await exists(name))
      result.protectedCandidates.push({
        path: name,
      ...
      if (data && typeof data === "object" && !Array.isArray(data)) {
        const scripts =
          ...
          !Array.isArray(data.scripts)
            ? data.scripts
            : {};
        for (const key of Object.keys(scripts).sort().slice(0, 32))
          if (typeof scripts[key] === "string")
            result.commands.push({
              name: key.slice(0, 80),
              source: "package.json#/scripts/" + key.slice(0, 80),
              bodyOmitted: true,
              executed: false,
            });
        ...
        result.dependencyEvidence = [
          ...
          .filter((k) => Object.hasOwn(deps, k))
          .map((name) => ({
            name,
            source: "package.json dependency key (not runtime verification)",
          }));
      }
    } catch {
      result.manifestWarning =
        "package.json unavailable, invalid or over 64KiB; no commands inferred";
    } finally {
      await handle?.close();
    }
  }
    ...
    try {
      const r = await exec(
        ...
        ],
        {
          ...
          timeout: Math.max(1, 5000 - (Date.now() - started)),
          maxBuffer: 16384,
          encoding: "utf8",
        },
      );
        ...
        reason:
          e.code === "ENOENT"
            ...
            : e.code === "ERR_CHILD_PROCESS_STDIO_MAXBUFFER"
              ? "Git output exceeded 16KiB"
              : "Git query unavailable (not a repository, permissions, or command failure)",
      };
    }
  };
  ...
  if (repo.ok) {
    result.git = {
      ...
      remoteState: "not contacted",
    };
    const [status, untracked, ignored, remotes, worktrees, hooks] =
      await Promise.all([
        git([
          "diff",
          "--cached",
    ...
    const [head, branch, commonDir, gitDir, upstream, divergence] =
      await Promise.all([
    ...
    result.git.versionControl = {
      head: head.ok ? head.text.trim() : null,
      branch: branch.ok ? branch.text.trim() : null,
      headState: branch.ok
        ? head.ok
          ? "attached"
          : "unborn"
      ...
      upstream: upstream.ok ? upstream.text.trim() : null,
      remoteFreshness:
        "local tracking refs only; no fetch, GitHub/CI state unverified",
      operations: [],
      collaboration:
        "Branches/refs are shared across linked worktrees; working files and indexes are per-worktree. Session checkpoints are not commits. Before staging, committing or changing branches, inspect current scope and peer work; verify the exact resulting commit, not just a branch name.",
    };
    if (divergence.ok && /^\d+\s+\d+\s*$/.test(divergence.text)) {
      const [ahead, behind] = divergence.text.trim().split(/\s+/).map(Number);
      result.git.versionControl.divergence = {
        ahead,
        behind,
        source: "local upstream tracking ref",
      };
    }
    if (gitDir.ok)
      for (const marker of [
        ...
        operationSignal.throwIfAborted();
        try {
          const stat = await fs.lstat(path.join(gitDir.text.trim(), marker));
          if (!stat.isSymbolicLink())
            result.git.versionControl.operations.push(marker);
        } catch {}
      }
    ...
    result.git.statusScope =
      "Staged, untracked and ignored paths only. Unstaged tracked contents and submodule contents NOT compared; not an atomic workspace snapshot.";
    result.git.trackedClean = null;
    const changes = [];
    const appendChange = (file, code) => {
      if (changes.length < 40)
        changes.push({
          path: file.slice(0, 240),
          status: code,
    ...
    if (status.ok) {
      ...
      for (let i = 0; i + 1 < records.length; i += 2)
        appendChange(records[i + 1], records[i] + " ");
      result.git.stagedClean = records.length === 0;
    } else {
      result.git.statusWarning = status.reason;
      result.truncated = true;
    }
    for (const [query, code] of [
      ...
      if (query.ok)
        for (const file of query.text.split("\0").filter(Boolean))
          appendChange(file, code);
      else {
        result.git.statusWarning = query.reason;
        result.truncated = true;
      }
    }
    result.git.changes = changes;
    result.git.remotes = remotes.ok
      ? remotes.text
          .split("\n")
          .filter(Boolean)
          .slice(0, 10)
      ...
      : [];
    if (!remotes.ok)
      result.git.remoteWarning =
        "No remote URLs returned (none configured or query unavailable)";
    if (remotes.ok && remotes.text.split("\n").filter(Boolean).length > 10)
      result.truncated = true;
    if (worktrees.ok) {
      const entries = worktrees.text
        .split("\0")
        .filter((x) => x.startsWith("worktree "));
      result.git.worktrees = entries
        .slice(0, 10)
        .map((x) => ({
          path: x.slice(9, 249),
          source: "git worktree list",
          isolation:
            "shares project Git database, not a separate harness repository",
        }));
      if (entries.length > 10) result.truncated = true;
    } else result.git.worktreeWarning = worktrees.reason;
    if (hooks.ok)
      result.git.hooksPath = {
        path: hooks.text.trim().slice(0, 240),
        source: "git rev-parse --git-path hooks",
  ...
  while (
    ...
    result.scope.candidates.length
  ) {
    result.scope.candidates.pop();
    result.truncated = true;
    result.scope.truncated = true;
  }
  for (const key of ["changes", "worktrees", "remotes"])
    while (JSON.stringify(result).length > 12000 && result.git[key]?.length) {
      result.git[key].pop();
      result.truncated = true;
    }
  while (JSON.stringify(result).length > 12000 && result.commands.length) {
    result.commands.pop();
    result.truncated = true;
  }
  return result;
...
export async function workspaceScope(root, signal) {
  const result = {
    ...
    probes: 0,
    policy:
      "Inspect related contracts and tests when relevant to the user task. A relationship is not permission to edit another folder. Confirm ownership before expanding edits; remote systems remain unverified.",
    limitations: [
      "Only bounded local manifest relationships and conventional module containers are inspected.",
      "Unsupported workspace globs and non-JSON dependency declarations require targeted inspection; absence is not proof of independence.",
    ],
  };
  const seen = new Set();
  const probe = async (file) => {
    signal?.throwIfAborted();
    if (result.probes >= 160) {
      result.truncated = true;
      return null;
    }
    result.probes++;
  ...
  for (const pattern of patterns(data).slice(0, 24)) {
    ...
    if (pattern.endsWith("/*")) {
      ...
      if (base)
        for (const name of await list(base))
          add(
            path.join(base, name),
            "declared workspace member",
            "package.json#/workspaces: " + pattern,
            true,
          );
    } else {
  ...
  for (const section of [
    ...
    for (const [name, value] of entries.slice(0, 128)) {
      ...
      if (!relative || relative.length > 240 || /[\x00-\x1f]/.test(relative))
        continue;
      add(
        path.resolve(root, relative),
        "declared local dependency (existence unverified)",
        "package.json#/" + section + "/" + name.slice(0, 100),
      );
    }
  }
  for (const container of ["apps", "packages", "services", "libs", "modules"]) {
    for (const name of await list(path.join(root, container))) {
      ...
      if (result.candidates.some((candidate) => candidate.path === dir))
        continue;
      for (const marker of [
        "package.json",
        "Cargo.toml",
        ...
        if ((await probe(path.join(dir, marker)))?.isFile()) {
          add(
            dir,
            "module candidate; task relevance unverified",
            path.relative(root, path.join(dir, marker)),
            true,
          );
          break;
        }
  ...
  for (
    let depth = 0;
    depth < 4 && parent !== path.dirname(parent);
    depth++, parent = path.dirname(parent)
  ) {
    const parentData = await json(parent),
      relative = path.relative(parent, root).split(path.sep).join("/");
    for (const pattern of patterns(parentData).slice(0, 24)) {
      if (!safePattern(pattern)) continue;
      const member = pattern.endsWith("/*")
        ? relative.startsWith(pattern.slice(0, -1)) &&
          relative.slice(pattern.length - 1).split("/")[0]
        : relative === pattern || relative.startsWith(pattern + "/");
      if (member) {
        add(
          parent,
          "declared containing workspace",
          path.join(parent, "package.json") + "#/workspaces: " + pattern,
          true,
        );
        break;
      }
...
const TEST_SOURCE =
  /\.(?:[cm]?[jt]sx?|py|rs|go|java|kt|kts|cs|fs|php|rb|swift|c|cc|cpp|cxx|h|hpp|sh|bash|ex|exs|erl|scala|clj|cljs|vue|svelte)$/i;
const TEST_PATH =
  /(?:^|\/)(?:tests?|__tests__|specs?)(?:\/|$)|(?:^|[._-])(?:test|spec)(?:[._-]|$)|_test\.[^/]+$/i;
const TEST_MANIFEST =
  /^(?:package\.json|pyproject\.toml|pytest\.ini|tox\.ini|Cargo\.toml|go\.mod|composer\.json|Gemfile|pom\.xml|build\.gradle(?:\.kts)?|CMakeLists\.txt|Makefile|.*\.(?:csproj|fsproj))$/;
const TEST_SKIP =
  /^(?:\..*|node_modules|vendor|target|dist|build|coverage|__pycache__|venv|env|logs?|artifacts?|backups?|sessions?|worktrees?|uploads?|storage|data|secrets?|credentials?)$/i;

export function isProjectTestSource(file) {
  return (
    !file.split(/[\\/]/).some((part) => TEST_SKIP.test(part)) &&
    TEST_SOURCE.test(file) &&
    !/\.min\.[cm]?js$/i.test(file)
  );
}

const reviewSkip = (part) =>
  (TEST_SKIP.test(part) || REVIEW_RUNTIME_DIR.test(part)) &&
  !/^(?:\.github|\.circleci|\.gitlab-ci\.yml)$/.test(part);
// Harness runtime state (reminder/checkpoint/catalog/health files) changes every
// turn. It is not project source: counting it kept bumping the review revision
// and invalidating otherwise-current reviews. Directory names are anchored so
// `extensions/reminders.ts` stays a source file.
const REVIEW_RUNTIME_DIR =
  /^(?:reminders?|checkpoints?|sibling-bridge|tasks?|memory|missions?|project-intelligence|npm|cache|inference-slots?)$/i;
const REVIEW_RUNTIME_STATE =
  /^(?:provider-health|free-route-evidence|live-model-catalog|models-store|run-history|session-report|notification-state)\.(?:json|jsonl)$/i;
const REVIEW_CONFIG =
  /\.(?:json|ya?ml|toml|xml|ini|cfg|conf|properties|txt|env)$/i;
const PRIVATE_REVIEW_CONFIG =
  /(?:^|[._-])(?:auth|settings|models|secrets?|credentials?|passwords?|tokens?|api[._-]?keys?|private[._-]?keys?)(?:[._-]|$)/i;
const DELIVERY_SOURCE =
  /^(?:(?:Dockerfile|Containerfile|Procfile)(?:\.[a-z0-9._-]+)?|[a-z0-9._-]+\.(?:Dockerfile|Containerfile)|Makefile|GNUmakefile|Jenkinsfile|Justfile)$/i;
export function isProjectReviewSource(file) {
  ...
  return (
    !parts.some(reviewSkip) &&
    !REVIEW_RUNTIME_STATE.test(basename) &&
...
export async function projectTestFacts(cwd, signal, options = {}) {
  ...
  const result = {
    ...
    entries: 0,
    limitations: [
      "No scripts/configs executed. Test filenames and script names do not establish coverage or safety.",
      "Source stat changes include shell/external writes; attribution and same-metadata changes are not established.",
      "Ignored/generated directories, symlinks and paths beyond scan limits are not inspected.",
    ],
  };
      ...
      for await (const entry of directory) {
        ...
        if (entry.isDirectory()) {
          ...
          if (TEST_MANIFEST.test(entry.name) && result.manifests.length < 32) {
            ...
            if (entry.name === "package.json" && manifests.length < 8)
    ...
    try {
      ...
      if (
        ...
        for (const name of Object.keys(data.scripts)
          .filter((name) => /^(?:test|check|verify)(?:$|[:-])/.test(name))
          .slice(0, 16)) {
          if (typeof data.scripts[name] === "string")
            result.scripts.push({
              manifest: path.relative(root, file),
