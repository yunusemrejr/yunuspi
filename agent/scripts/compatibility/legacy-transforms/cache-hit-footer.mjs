...
const previousSessionCostSource =
...
const releasedSessionCostSource =
...
const costHelpers = ["cost-evidence.ts", "session-cost.ts"]
  .map((name) =>
    fs
      .readFileSync(
        new URL("../../../extensions/lib/" + name, import.meta.url),
        "utf8",
      )
      .replace(/^import .*;\n/gm, "")
  ...
  .join("\n");
export const sessionCostSource =
  "function formatSessionCost(entries, subscription) {\n" +
  costHelpers +
  "\nreturn collectSessionCost(entries,subscription).formatted;\n}";

export const activitySource = fs
  .readFileSync(
    new URL("../../../extensions/lib/session-metrics.ts", import.meta.url),
    "utf8",
  )
  .replace(
...
const legacyActivityHash =
  "67ccc1390268047f39ce9cf6985d22bbd73f0530606d98aa5c0fcc203e4cd07b";
...
function activityCode(helper, bundled, version) {
  ...
  return (
    (bundled ? ";" : "") +
    `${activityPrefix}${helper}${activitySuffix}
let activityLine = '';
...
/* PI_SESSION_ACTIVITY_V${version} */ ` +
    (bundled
      ? "let extensionStatuses=this.footerData.getExtensionStatuses()"
      : old)
  );
}
export function transformActivity(source, bundled) {
  ...
  if (source.includes("PI_SESSION_ACTIVITY_V2") && !source.includes(code)) {
    const start = source.indexOf(activityPrefix),
      end = source.indexOf(activitySuffix, start + activityPrefix.length);
    ...
    if (
      [
        ...
        "61c16b6a591f8438e1fca219d159cf80e4f95a8ebd78aa80dc8220d731f6e4a7",
        "b2171c708285aa3bf92c1ada0ab6a69fe0195b7e27d49e68b81f9f28ed62bc34",
      ...
      if (source.split(old).length === 2)
        source = source.replace(old, () => code);
    }
  }
  if (source.includes("PI_SESSION_ACTIVITY_V2")) {
    if (
      source.split(code).length !== 2 ||
      source.includes("PI_SESSION_ACTIVITY_V1")
    )
      throw new Error("session activity postcondition drift");
    return source;
  }
  if (source.includes("PI_SESSION_ACTIVITY_V1")) {
    ...
    if (start < 0 || end < 0)
      throw new Error(
        "session activity V1 migration drift: missing helper boundary",
      );
    const helper = source.slice(start + activityPrefix.length, end);
    if (
      createHash("sha256").update(helper).digest("hex") !== legacyActivityHash
    )
      throw new Error(
        "session activity V1 migration drift: unknown helper payload",
      );
    const previous = activityCode(helper, bundled, 1);
    if (
      source.split(previous).length !== 2 ||
      source.split("PI_SESSION_ACTIVITY_V1").length !== 2
    )
      throw new Error("session activity V1 migration drift: insertion changed");
    return source.replace(previous, () => code);
  }
  if (source.split(old).length !== 2)
    throw new Error("session activity footer anchor drift");
...
export function transformFooter(source, bundled = false) {
  ...
  if (source.includes(legacyCall))
    source = source.replace(legacyCall, () => call);
...
function transformDisplay(source, bundled) {
  for (const old of [
    previousSessionCostSource,
    legacySessionCostSource,
    releasedSessionCostSource,
  ])
    if (source.includes(old))
      source = source.replace(old, () => sessionCostSource);
  const begin = source.indexOf("function formatSessionCost(");
  const end = source.indexOf(
    ")(this.session.sessionManager.getEntries(), usingSubscription)",
    begin,
  );
  if (begin >= 0 && end > begin) {
    ...
    if (
      [
        "1603856a11afd1f7d74071948742364bcaa53d98404dbe2cde0d83ebeb942fc6",
        "4d901db2e4f55ec9d803d1445162365b513d630c21ad50652fa3ef1f35b36df3",
      ].includes(createHash("sha256").update(old).digest("hex"))
    )
  ...
  const pairs = [
    ...
    [
      ...
      bundled
        ? 'contextPercent=Number.isFinite(contextUsage?.percent)?contextPercentValue.toFixed(1):"?"'
    ...
    [
      ...
      bundled
        ? 'statsLeft=statsParts.sort((a,b)=>(a===contextPercentStr?0:a.startsWith("CH")?1:a.startsWith("$")||a==="sub"?2:3)-(b===contextPercentStr?0:b.startsWith("CH")?1:b.startsWith("$")||b==="sub"?2:3)).join(" ")'
