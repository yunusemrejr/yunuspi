...
function parseArgs(argv) {
	...
	for (let i = 0; i < argv.length; i++) {
		...
		else if (arg === "--metadata" && argv[i + 1])
			metadata = path.resolve(argv[++i]);
		else if (arg === "--check") check = true;
		else
			throw new Error(
				"Usage: generate-capabilities-doc.mjs --root ROOT [--output FILE] [--metadata FILE] [--check]",
			);
	}
...
function importCatalog(catalogPath) {
	...
	if (result.status !== 0)
		throw new Error(
			`Unable to load harness-capabilities.ts: ${result.stderr || result.stdout}`,
		);
	try {
		...
		throw new Error(
			`Unable to parse harness capability catalog: ${error.message}`,
		);
	}
}

function commandSourceInventory(agentRoot) {
	const sourceFiles = walkFiles(path.join(agentRoot, "extensions")).filter(
	...
	for (const file of sourceFiles) {
		...
		for (let index = 0; index < lines.length; index++) {
			...
			const match =
				/\b(?:pi\.)?registerCommand\s*(?:\?\.)?\s*\(\s*(['"])([^'"\r\n]+)\1/.exec(
					line,
				);
			if (match)
				commandRegistrations.push({ name: match[2], source, line: index + 1 });
			else if (/\b(?:pi\.)?registerCommand\s*(?:\?\.)?\s*\(/.test(line))
				addDynamic(
					commandDynamicOwners,
					"registerCommand() receives a computed name",
					index + 1,
				);
		}
	}
...
function serviceOwners(agentRoot) {
	const files = walkFiles(path.join(agentRoot, "extensions")).filter((file) =>
	...
	for (const file of files) {
		...
		if (
			/(?:^|[/\\])[^/\\]*mcp[^/\\]*(?:[/\\]|$)/i.test(rel(agentRoot, file)) ||
			/\bMCP\b|mcpConfig|mcpDirectTools|mcp-server|mcp server/i.test(text)
		)
			signals.add("MCP");
		if (
			/(?:wrapper|adapter|client)/i.test(path.basename(file)) ||
			(/\b(?:wrapper|adapter|client)\b/i.test(text) &&
				/registerTool|spawn\(|fetch\(/.test(text))
		)
			signals.add("wrapper/adapter");
...
function manifestInventory(agentRoot, manifest) {
	...
	const list = (values, base = "") =>
		Array.isArray(values)
			? values
					.map((value) => existing(value, base))
					.sort((a, b) => a.path.localeCompare(b.path))
			: [];
	const forks = Object.entries(manifest.forks ?? {})
		.map(([key, packageName]) => ({
	...
	return {
		...
		foldedMarkers: (manifest.foldedMarkers ?? [])
			.map((item) => ({ ...item }))
		...
		retired: (manifest.retired ?? [])
			.map((item) => ({ name: item.name, retired: item.retired, why: item.why }))
	...
	return walkFiles(docsRoot)
		.filter(
			(file) =>
				...
				path.basename(file) !== "CAPABILITIES.md",
		)
		.map((file) => ({
			name: path.basename(file),
			path: canonicalTemplatePath(docsRoot, file),
		}))
		.sort((a, b) => a.path.localeCompare(b.path));
}
...
function resolveReference(layout, reference) {
	...
	if (value.startsWith("agent/public-template/docs/"))
		return path.join(
			layout.docsRoot,
			value.slice("agent/public-template/docs/".length),
		);
	if (value.startsWith("agent/public-template/"))
		return path.join(
			layout.templateRoot,
			value.slice("agent/public-template/".length),
		);
	if (value.startsWith("agent/")) return path.join(layout.projectRoot, value);
	if (value.startsWith("docs/"))
		return path.join(layout.docsRoot, value.slice("docs/".length));
	if (
		value.startsWith("extensions/") ||
		value.startsWith("scripts/") ||
		value.startsWith("skills/")
	)
		return path.join(layout.agentRoot, value);
...
function link(layout, output, reference, label = reference) {
	...
	if (!fs.existsSync(target))
		return `\`${label}\` (missing from sanitized source)`;
...
function capabilityMarkdown(
	...
	for (const group of [...groups.keys()].sort()) {
		...
		for (const record of groups.get(group)) {
			...
			if (record.entrypoints?.length)
				lines.push(
					`**Entrypoints:** ${record.entrypoints.map((x) => `\`${x}\``).join(", ")}`,
					"",
				);
			if (record.tools?.length)
				lines.push(
					`**Catalog tool pointers:** ${record.tools.map((x) => `\`${x}\``).join(", ")}`,
					"",
				);
			if (record.commands?.length)
				lines.push(
					`**Commands:** ${record.commands.map((x) => `\`/${x}\``).join(", ")}`,
					"",
				);
			if (record.options?.length) {
				lines.push(
					...
					...record.options.map(
						(item) =>
							`- \`${item.name}\`: ${item.summary}${item.values?.length ? ` Values: ${item.values.map((x) => `\`${x}\``).join(", ")}.` : ""}`,
					),
					"",
				);
			}
			if (record.related?.length)
				lines.push(
					`**Related records:** ${record.related.map((x) => `\`${x}\``).join(", ")}`,
					"",
				);
			if (record.sourceFiles?.length)
				lines.push(
					`**Source:** ${record.sourceFiles.map((x) => link(layout, output, x)).join(", ")}`,
					"",
				);
			if (record.doc)
				lines.push(
					`**Documentation:** ${link(layout, output, record.doc, record.doc.replace(/^agent\/public-template\/docs\//, "docs/"))}`,
					"",
				);
		}
	}
	...
	lines.push(
		...
		"### Native core tools",
		"",
		tools.nativeCore.map((name) => `\`${name}\``).join(", ") +
			". Extensions can wrap or replace these; explicit tool selections still apply.",
		"",
	...
	lines.push(
		listItems(
			tools.stableRegistrations,
			(item) =>
				`\`${item.name}\` — ${link(layout, output, item.source)} (line ${item.line}; ${item.kind})`,
		),
		"",
		"### Dynamic tool owners",
		"",
		listItems(
			tools.dynamicOwners,
			(item) =>
				`${link(layout, output, item.source)} — ${item.signals.join("; ")}${item.knownNames?.length ? `; known tools: ${item.knownNames.map((name) => `\`${name}\``).join(", ")}` : ""}${item.lines.length ? ` (lines ${item.lines.join(", ")})` : ""}`,
		),
		"",
		"### Literal slash commands",
		"",
		listItems(
			tools.commandRegistrations,
			(item) =>
				`/${item.name} — ${link(layout, output, item.source)} (line ${item.line})`,
		),
		"",
		"### Dynamic command owners",
		"",
		listItems(
			tools.commandDynamicOwners,
			(item) =>
				`${link(layout, output, item.source)} — ${item.signals.join("; ")}${item.lines.length ? ` (lines ${item.lines.join(", ")})` : ""}`,
		),
		"",
	);
	...
	lines.push(
		listItems(
			inventory.serviceOwners,
			(item) =>
				`${link(layout, output, item.source)} — ${item.signals.map((x) => `\`${x}\``).join(", ")}`,
		),
		"",
	);
	...
	lines.push(
		...
		listItems(
			...
			(item) =>
				`${link(layout, output, item.path)}${item.exists ? "" : " — missing from source tree"}`,
		),
		"",
		"### Libraries",
		"",
		...
		listItems(
			inventory.forks,
			(item) =>
				`${link(layout, output, item.path)} — package \`${item.package}\`${item.exists ? "" : " — missing from source tree"}`,
		),
		"",
		...
		"",
		listItems(
			inventory.foldedMarkers,
			(item) => `\`${item.file}\` — ${item.label} (marker \`${item.marker}\`)`,
		),
		"",
		"### Vendored runtime dependencies",
		"",
		listItems(inventory.vendoredDependencies, (item) => `\`${item}\``),
		"",
		"### Extension packages",
		"",
		listItems(inventory.npmPackages, (item) => `\`${item}\``),
		"",
	);
	...
	lines.push(
		"## Skills",
		"",
		`The exporter includes ${skills.length} public skill directories. This list is a path inventory; skill contents remain in their linked \`SKILL.md\` files.`,
		"",
		listItems(
			skills,
			(item) => `\`${item.name}\` — ${link(layout, output, item.path)}`,
		),
		"",
	);
	lines.push(
		...
		listItems(
			...
			(item) =>
				`${link(layout, output, item.path)} (source type \`${item.kind}\`)`,
		),
		"",
	);
	...
	lines.push(
		...
		"",
		listItems(docs, (item) => link(layout, output, item.path, item.path)),
		"",
	);
	lines.push(
		...
		listItems(
			inventory.retired,
			(item) => `\`${item.name}\` — retired ${item.retired}: ${item.why}`,
		),
		"",
	);
	return (
...
function build(layout, output, metadataPath) {
	...
	if (!fs.existsSync(manifestPath) || !fs.existsSync(catalogPath))
		throw new Error("Public manifest or capability catalog is missing");
	...
	tools.literalRegistrations = stableTools.stableRegistrations.filter(
	...
	const docs = docsInventory(
		layout.docsRoot ?? path.join(layout.templateRoot, "docs"),
	);
	const metadata = {
		schemaVersion: 1,
		...
		source: {
			...
			toolEvidence:
				"literal registrations, source-owned factories/catalogs/constants, native core names and explicitly named dynamic owners",
		},
		capabilities: records.map((record) => ({
			...record,
			doc: record.doc.replace(/^agent\/public-template\//, ""),
			sourceFiles: record.sourceFiles.map((file) =>
				file.replace(/^agent\/public-template\//, ""),
			),
		})),
		tools,
		inventory: { ...inventory, skills, documentation: docs },
	};
	const markdown = capabilityMarkdown(
		layout,
		output,
		records,
		tools,
...
function main() {
	...
	const outputs = args.output
		? [
				{
					output: args.output,
					metadata: args.metadata ?? args.output.replace(/\.md$/i, ".json"),
				},
			]
		: [
				{
					output: defaultOutput,
					metadata: defaultOutput.replace(/\.md$/i, ".json"),
				},
			];
	...
	if (
		!args.output &&
		path.basename(layout.templateRoot) === "release-template"
	) {
		const rootOutput = path.join(layout.projectRoot, "docs", "CAPABILITIES.md");
		if (!outputs.some((item) => item.output === rootOutput))
			outputs.push({
				output: rootOutput,
				metadata: rootOutput.replace(/\.md$/i, ".json"),
			});
	}
	const generated = [];
	for (const item of outputs) {
		const targetLayout = layoutForOutput(layout, item.output);
		const { markdown, metadata } = build(
			targetLayout,
			item.output,
			item.metadata,
		);
		if (args.check) {
			const actualMarkdown = fs.existsSync(item.output)
				? fs.readFileSync(item.output, "utf8")
				: undefined;
			const actualMetadata = fs.existsSync(item.metadata)
				? fs.readFileSync(item.metadata, "utf8")
				: undefined;
			if (actualMarkdown !== markdown || actualMetadata !== metadata) {
				console.error(
					`Capability inventory is stale: ${item.output}. Run: node agent/scripts/generate-capabilities-doc.mjs --root ${args.root}`,
				);
				process.exitCode = 1;
			}
		} else {
