...
export function parseSelection(args, tests) {
  ...
  let jobs = 1,
    report,
    list = false,
    timeoutMs = DEFAULT_SUITE_DEADLINE_MS;
  for (let i = 0; i < args.length; i++) {
    ...
    if (key === "--test" && tests.includes(value)) requested.push(value);
      ...
      if (!matches.length)
        throw new Error(
          `No registered offline suites match ${JSON.stringify(value)}. Use --list to inspect available suites.`,
        );
      requested.push(...matches);
    } else if (key === "--jobs" && /^[1-4]$/.test(value ?? ""))
      jobs = Number(value);
    else if (key === "--timeout" && /^\d+$/.test(value ?? "")) {
      const seconds = Number(value);
      if (seconds < 1 || seconds * 1000 > MAX_SUITE_DEADLINE_MS)
        throw new Error(
          `Invalid --timeout ${value}: use whole seconds between 1 and ${MAX_SUITE_DEADLINE_MS / 1000}. Longer deadlines are deliberate, not a default.`,
        );
      timeoutMs = seconds * 1000;
    } else if (key === "--report" && value && !value.startsWith("--"))
      report = path.resolve(value);
    else
      throw new Error(
        `Invalid test selection: ${value ?? key}. Use --test <registered relative path>, --match <literal path text>, --list, --jobs <1-4>, --timeout <1-600 seconds>, or --report <file>.`,
      );
  }
  return {
    selected: [...new Set(requested.length ? requested : tests)],
    jobs,
    report,
    list,
    timeoutMs,
...
export function runSuite(
  test,
  file,
  { signal, env = process.env, cwd, timeoutMs = 120000 } = {},
) {
  if (
    !Number.isSafeInteger(timeoutMs) ||
    timeoutMs < 1 ||
    timeoutMs > MAX_SUITE_DEADLINE_MS
  )
    throw new Error("Invalid suite deadline");
  ...
  return new Promise((resolve) => {
    ...
    const deadline = setTimeout(() => {
      deadlineFired = true;
      error ??= "test exceeded suite deadline";
      stop();
    }, timeoutMs + 25);
    const abort = () => {
    ...
    const collect = (stream) => (data) => {
      ...
      if (bytes > 2 * 1024 * 1024 && !error) {
        error = "test output exceeded 2 MiB";
        stop();
      }
    };
  ...
  await Promise.all(
    Array.from({ length: Math.min(jobs, selected.length) }, async () => {
      while (next < selected.length && !signal?.aborted) {
        const index = next++;
        results[index] = await run(selected[index]);
      }
    }),
  );
