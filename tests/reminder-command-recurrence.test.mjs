...
const { readRemindersState } = await import(extension("lib/reminders-state.ts"));
const T = 120_000;
...
function fixture(sid, { reject = false, rejectNotify = false } = {}) {
	...
	return {
		...
		setReject: (value) => { shouldReject = value; },
...
try {
	...
	assert.equal(first.sent.length, 1, "slash command sends the first reminder immediately");
	assert.equal(first.sent[0].options.deliverAs, "steer");
	assert.equal(first.sent[0].options.triggerTurn, true, "explicit command may wake an idle agent once");
	assert.equal(first.sent[0].message.display, true);
	assert.ok(first.sent[0].message.content.includes(text), "immediate delivery preserves the complete accepted text");
	...
	// anchored two-minute point and never from a final/idle turn.
	...
	assert.equal(first.sent.length, 2, "first scheduled repeat reaches an active tool turn");
	assert.ok(first.sent[1].message.content.includes(text), "scheduled repeat preserves the complete accepted text");
	...
	assert.equal(first.sent.length, 3, "second scheduled repeat remains available");
	...
	const resumedStart = await resumed.emit("before_agent_start", { prompt: "Continue the task" });
	assert.equal(resumed.sent.length, 0, "resume injects through before_agent_start without a wake");
	assert.match(resumedStart?.message?.content ?? "", /FINAL-VERIFICATION-CLAUSE/);
	state = readRemindersState("command-recurrence");
	assert.equal(state.manual[0].delivered, 4);

	await resumed.command("clear 1");
	state = readRemindersState("command-recurrence");
	assert.equal(state.manual[0].active, false, "clear completes the retained reminder");
	...
	assert.ok(rejected.notices.some(({ message, level }) => level === "warning" && /immediate delivery failed/.test(message)));
	state = readRemindersState("command-rejected");
	assert.equal(state.manual[0].delivered, 0);
	assert.equal(state.manual[0].nextFireAt, rejectedAt, "failed first send is retained as due");
	rejected.setReject(false);
	now += T;
	const retry = await rejected.emit("before_agent_start", { prompt: "Continue" });
	assert.equal(rejected.sent.length, 0, "retry uses the normal prompt injection path");
	...
	const notifyFailure = fixture("command-notify-failure", { rejectNotify: true });
	...
	assert.equal(readRemindersState("command-notify-failure").manual[0].delivered, 1);

	console.log("PASS reminder-command-recurrence: immediate delivery, active-turn repeats, resume, clear, and retry");
} finally {
	Date.now = oldNow;
