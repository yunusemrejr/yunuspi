...
const moduleUrls = {
  "@earendil-works/pi-coding-agent": pathToFileURL(
    path.join(core, "dist/index.js"),
  ).href,
  "@earendil-works/pi-ai": pathToFileURL(
    path.join(core, "node_modules/@earendil-works/pi-ai/dist/index.js"),
  ).href,
};
register(
  `data:text/javascript,${encodeURIComponent(
    `const urls=${JSON.stringify(moduleUrls)};export function resolve(name,context,next){return urls[name]?{url:urls[name],shortCircuit:true}:next(name,context);}`,
  )}`,
  import.meta.url,
);
