...
| `image_create` | Synthesizes a deterministic procedural plate from explicit parameters: solid fills, multi-stop linear gradients, transparency checkerboards, seeded film grain and grid paper. Same inputs render the same bytes; one PNG/JPEG/WebP up to 16M pixels. Text, logos and photographic content stay in SVG/HTML rendered by `render_see`. |
