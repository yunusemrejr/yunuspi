...
| Local LM lines | Successful bash command output can become a validated source-linked line selection by the local Qwen3.5-0.8B model, including windowed head/diagnostics/tail for 4–32 KiB outputs. Judgements (skill relevance, mutation pre-screen) reuse a server-side checkpoint of their few-shot prefix: ~0.2 s instead of ~1.1 s. | `read`/`grep`/`find`/`ls` and bash file views (`cat`, `sed -n`, `grep`, `git show`…) are requested content and stay exact. Stack traces, patches, secrets and instruction-like text abstain; so does UNKNOWN. Boundary lines, the four strongest task matches and every distinct status fact or non-template qualified fact are retained; the original stays behind `obs_read`. |
| Skill-hint gate | Catalog-wide "session context" skill hints pass a calibrated local relevance judgement for the current request before delivery. | Hints need a non-generic term; without the model the lexical hint keeps its previous behavior. |
| Evidence routing | Deterministic content-shape classification routes each substantial result to the local LM, Kompress, Needle, and/or Jev. | The deterministic distiller runs first and its win ends routing. Not every helper runs on every result. |
| Intent screen | A Needle → local LM → Jev pre-screen can resolve task-mutation rescue checks before the full-LLM arbiter. | Asymmetric bars: the fail-safe direction decides loosely, rescue decides strictly, everything else defers. |
...
Kompress is a token classifier, so task conditioning happens around its source-ID proposal. Prepending an instruction prompt would change its input distribution without demonstrating better selection. The model's own [documentation](https://huggingface.co/chopratejas/kompress-small) describes English prose and sequence-length limits. The local language model (Qwen3.5-0.8B) selects lines from safe line-oriented output and answers calibrated yes/no judgements (skill relevance, the fail-safe side of the mutation screen); generation, factual summaries, coding and correctness judgments are still not assigned to it. See [MICRO-INTELLIGENCE.md](MICRO-INTELLIGENCE.md#local-language-model--judgements-and-line-selection).
...
`PI_LOCAL_INTELLIGENCE=off` disables the new selection and reranking paths; established context scoring and provider-health protection remain available. `PI_HANDOFF_SELECTION=off` disables child report selection alone. Existing `PI_MINI_PREPROCESSOR`, `PI_CONTEXT_MEMORY` and `PI_OUTPUT_DISTILLER` controls retain their scopes. New controls: `PI_NEEDLE=off` and `PI_NEEDLE_SHADOW=1` (disable / shadow-measure Needle), `PI_INTENT_PRESCREEN=off` (intent pre-screen), `PI_MICRO_INTELLIGENCE=off` (lifecycle extension). Install changes through the normal update/reload workflow; running sessions need to load the updated extensions.

The public `tests/local-intelligence.test.mjs` uses synthetic fixtures to test task switches, cache isolation, exact source reconstruction, critical overflow, compaction input reduction, saved-file handoffs, sparse metric changes, error polarity and stale telemetry. Existing context, observation, skill, graph, model-quality and recovery regressions cover integration. Needle, coordination, expansion, session, and calibration coverage lives in `tests/needle-*.test.mjs`, `tests/micro-intelligence.test.mjs`, `tests/smol-kompress-expansion.test.mjs`, `tests/local-lm.test.mjs`, `tests/micro-intel-session.test.mjs`, and `tests/micro-intel-bench.test.mjs` with fixtures in `tests/fixtures/micro-intel/`. These tests establish their contracts, not broad model intelligence or measured production token savings. Optional local Kompress weights, the local language model and their service availability are separate from distribution tests (which run with `PI_LOCAL_LM=off`).
...
Named helper activity is UI-only and uses the existing theme colors. Completion remains visible beside ongoing work; errors and canceled work have distinct labels. The footer wraps named powers and status text at narrow terminal widths.

Kompress warms once when its shared service starts, before accepting requests. Clients no longer consume the shared ten-second inference slot for redundant warmups. Refusals made before inference (shape, size or token window) spend no slot and, when the worker marks them with `X-Kompress-Inference: 0`, no client cooldown either. Other valid refusals and busy responses keep the ordinary cooldown and appear as skipped work; malformed responses and timeouts appear as errors. Output routes to Kompress only when a selection could pay for itself; the rest stays available to the local LM and Jev. See [MICRO-INTELLIGENCE.md](MICRO-INTELLIGENCE.md#kompress--extractive-prose-selection) for the Markdown and Unicode eligibility rules and their measurement.
