...
| Local LM lines | Bounded successful line output can become a validated source-linked line selection by the local Qwen3.5-0.8B model, including windowed head/diagnostics/tail for 4–32 KiB outputs. | Stack traces, patches, secrets and instruction-like text abstain. Boundary, task and every distinct status line are retained; the original stays behind `obs_read`. |
