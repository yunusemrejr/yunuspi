...
- `scope`: require each named file to exist after work. For HTML and static PHP source, preserve the pre-existing H1 count and reject increased orphan/unclosed counts for definite container tags. A fragment with no H1 is allowed. Existing balance defects are compared with the baseline rather than blamed on the new work. PHP code is masked up to `?>` or the end of the file (the closing tag is conventionally omitted), while heredoc and nowdoc template bodies stay visible as HTML; before this, a balanced template after `<?php … <<<'HTML'` was read as an orphan `</div>` and rejected finished work. Successful native `write`/`edit` tool calls outside this scope reject completion.
