...
The example configuration uses `compaction.maxContextTokens: 96000` instead of 256000. Existing headroom policy compacts before that configured ceiling; recent-tail settings, model limits, summary validation and raw session history still apply. Installers preserve existing user settings. This bounds growth; it is not a measured guarantee about answer quality or total session tokens. `PI_ACTION_PLAN=off` disables the automatic plan orientation, while the todo tool remains available. Existing local model installation and eligibility requirements still apply; public source does not ship model weights or enable an unavailable worker.
