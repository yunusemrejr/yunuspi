...
For an exploratory check that a peer might otherwise repeat, call `session_coordinate({action:"prepare_check",checkName:"Parser fixture",command:"node --test tests/parser.test.mjs",files:["src/parser.js","tests/parser.test.mjs"]})`. Wait for preparation to complete, then invoke that exact command through the normal `bash` tool in a later tool batch. Preparation never executes anything or bypasses shell authorization. It expires after ten minutes, accepts one pending check and up to eight regular checkout files of at most 256 KiB each, and is cancelled by a different next bash command, new user input, session transition or shutdown. Native blocks and aborts settle the preparation rather than leaving a phantom running check.

The existing coordination record retains at most four bounded receipts. Each cites the publishing session and native `toolCallId`, actual command hash, duration, and `exit-zero`, `error` or `unverified` outcome. A missing native exit status, rewritten command or different execution directory cannot become `exit-zero`. Status rechecks named source versions and reports `current`, `changed` or `unverifiable`; changed inputs never lose their failure evidence. Exact versions remain in session entries and tool details, while the model receives concise relative paths. Receipt freshness checking is capped at 32 unique files per status call. There is no background polling, additional inference or automatic peer wait.

These receipts are untrusted peer reports with native references, not test-coverage certificates. Exit zero does not prove that a command exercised the named files; inspect its originating command and result before relying on it. Before/after snapshots do not lock files during execution or cover undeclared dependencies, environment, services or external data. Required checks remain required. Check names, outcomes, automatic plans and recent-write evidence cannot be forged through extra `publish` arguments.
