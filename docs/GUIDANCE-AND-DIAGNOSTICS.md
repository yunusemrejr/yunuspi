...
Typed mechanical judgments can use TypeSafe Jev through the configured OpenRouter key. Consumers retain their heuristic path when confidence is low or inference is unavailable; controlled success and skip reasons feed the TUI and `/used` telemetry. Identical state and questions share a request and cached answer across caller sites in the same process. A cancelled waiter does not cancel other waiters; one surviving caller records the paid result and the others record zero-cost reuse. Slug failover, a five-minute breaker and bounded requests avoid repeated failing work. These are client usage estimates, not invoice evidence; `PI_JEV=off` disables remote calls. Local confidence never establishes completion or replaces native compaction history.
