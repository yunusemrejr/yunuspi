...
Accepted user input, session replacement and model changes always invalidate old observations. Freshness is checked against the evidence actually cited: changed running/todo/child/completed state, newly overlapping tool resources or action topics, and lost intervening events reject a note. Unrelated tool progress can proceed without discarding useful advice. Notes include their reviewed snapshot identifiers and require checking newer work. Resource/topic matching is a conservative heuristic, not a semantic proof; related fast-changing work may still discard a suggestion rather than present a false premise. Unconsumed advice expires after 120 seconds and is checked again before delivery. Context advice remains optional and cannot act as a user request or permission.
